...
test("authenticated operator can reach production work", async ({ page }) => {
  ...
  await expect(page.locator("section.work-page select").first().locator("option")).toHaveCount(2, { timeout: 15_000 });
...
test("authenticated operator can switch locale and theme", async ({ page }) => {
  ...
  await expect.poll(() => page.evaluate(() => document.documentElement.scrollWidth - window.innerWidth)).toBeLessThanOrEqual(1);
  expect(await page.locator(".quality-page fieldset").evaluate((el) => el.getBoundingClientRect().right <= document.documentElement.clientWidth + 1)).toBeTruthy();
