...
async function signInThroughIdentity(page) {
  ...
  if (await email.count()) {
    ...
    await page.waitForURL((url) => url.origin === new URL(APPS.clinical).origin && url.pathname === '/en/dashboard', { timeout: 30000 });
  }
  await expect(page).toHaveURL((url) => url.origin === new URL(APPS.clinical).origin && url.pathname === '/en/dashboard', { timeout: 30000 });
