...
Describe 'compose.runtime.env.ps1' {
    It 'renders a non-secret compose adapter file for development' {
        ...
        $values['DATABASE_IDENTITY_URL'] | Should -Be 'Host=postgres;Database=identitydb;Username=postgres'
        $values['REDIS_URL'] | Should -Be 'redis:6379'
        $values['RABBITMQ_URL'] | Should -Be 'rabbitmq'
        $values.Keys | Should -Not -Contain 'SECRET_POSTGRES_PASSWORD'
        $values.Keys | Should -Not -Contain 'SECRET_RABBITMQ_PASSWORD'
        $values.Keys | Should -Not -Contain 'SECRET_REDIS_PASSWORD'
    }
