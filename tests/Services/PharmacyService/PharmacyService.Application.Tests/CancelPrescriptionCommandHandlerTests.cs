...
public class CancelPrescriptionCommandHandlerTests
{
    ...
    private readonly CancelPrescriptionCommandHandler _handler;

    public CancelPrescriptionCommandHandlerTests()
    {
        ...
        _handler = new CancelPrescriptionCommandHandler(_mockRepository.Object);
    }

    [Fact]
    public async Task Handle_WithValidIdAndReason_ShouldCancelPrescription()
    {
        ...
        var command = new CancelPrescriptionCommand(prescriptionId, "Patient request");

        var result = await _handler.Handle(command, CancellationToken.None);

        result.Should().Be(Unit.Value);
        prescription.Status.Should().Be(PrescriptionStatus.Cancelled);
        prescription.CancellationReason.Should().Be("Patient request");
        prescription.CancelledDate.Should().NotBeNull();
    ...
    public async Task Handle_WhenNotFound_ShouldThrow()
    {
        ...
        var command = new CancelPrescriptionCommand(prescriptionId, "Reason");
