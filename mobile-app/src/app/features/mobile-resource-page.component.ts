...
@Component({
  ...
  template: `
    <section class="mobile-page" [attr.aria-busy]="loading">
      <hh-toolbar class="mobile-page__toolbar" [label]="title + ' controls'">
        ...
        <button
          hh-toolbar-actions
          class="hh-icon-button mobile-page__refresh"
          type="button"
          (click)="load()"
          [disabled]="loading"
          [attr.aria-label]="'Refresh ' + title"
          [attr.title]="'Refresh ' + title"
        >
          <hh-mobile-icon
            name="refresh"
            [class.mobile-page__refresh--spinning]="loading"
          />
        </button>
        <button
          hh-toolbar-actions
      ...
      <hh-mobile-refresher (refreshed)="load()">
        <hh-mobile-infinite-list
          ...
          (loadMoreRequested)="loadMore($event.cursor)"
        >
          <hh-data-table
            [label]="title"
            [columns]="columns"
            ...
            (bulkActionRequested)="bulk($event)"
      ...
      <hh-mobile-action-sheet
        ...
        ><button
          ...
          (click)="actionsOpen = false; load()"
        >
          <hh-mobile-icon name="refresh" />Refresh list
        </button></hh-mobile-action-sheet
      >
  ...
  styles: [
    `
      ...
      :host ::ng-deep .mobile-page__toolbar .hh-toolbar {
        flex-wrap: nowrap;
        gap: 8px;
        margin-bottom: 0;
      }
      :host ::ng-deep .mobile-page__toolbar .hh-toolbar__title {
        flex: 1 1 auto;
        min-width: 0;
      }
      :host ::ng-deep .mobile-page__toolbar .hh-toolbar__actions {
        flex: 0 0 auto;
        width: auto;
        margin-left: auto;
      }
      .mobile-page__toolbar {
        position: sticky;
      ...
      .mobile-page__refresh:disabled {
        opacity: 0.55;
        cursor: wait;
      }
      .mobile-page__refresh--spinning {
...
export class MobileResourcePageComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.resource = this.route.snapshot.data["resource"] as MobileResource;
    this.configure();
    this.load();
  }
  ...
  configure(): void {
    const configs: Record<
      MobileResource,
      { title: string; columns: HisHopeDataTableColumn[]; empty: string }
    > = {
      clients: {
        title: "Clients",
        empty: "No OIDC clients found.",
        columns: [
          {
            key: "clientId",
            label: "Client ID",
          ...
          { key: "clientType", label: "Type", responsivePriority: 2 },
          {
            key: "redirectUris",
            label: "Redirect URIs",
      ...
      users: {
        ...
        columns: [
          ...
          { key: "roles", label: "Roles", responsivePriority: 3 },
          {
            key: "isActive",
            label: "Active",
            responsivePriority: 2,
      ...
      roles: {
        ...
        columns: [
          { key: "name", label: "Name", sortable: true, responsivePriority: 1 },
          { key: "description", label: "Description", responsivePriority: 2 },
        ],
      },
      consents: {
        title: "Consents",
        ...
        columns: [
          ...
          { key: "clientId", label: "Client ID", responsivePriority: 2 },
          { key: "scopes", label: "Scopes", responsivePriority: 3 },
          {
            key: "created",
            label: "Created",
            sortable: true,
    ...
    const config = configs[this.resource];
    this.title = config.title;
    this.emptyMessage = config.empty;
  ...
  load(query = this.query): void {
    ...
    this.api
      .getPage<MobileClient | MobileUser | MobileRole | MobileConsent>(
        this.resource,
        this.query,
      )
      .pipe(
        ...
        catchError(() => {
          if (sequence === this.requestSequence)
            this.error = `Unable to load ${this.title.toLowerCase()}.`;
          return of(null);
        }),
      )
      .subscribe((result) => {
        if (!result || sequence !== this.requestSequence) {
          this.changeDetector.detectChanges();
          return;
        }
  ...
  loadMore(cursor: string | null = this.nextCursor): void {
    ...
    this.api
      ...
      .pipe(
        ...
        catchError(() => {
          if (sequence === this.requestSequence)
            this.loadMoreError = `Unable to load more ${this.title.toLowerCase()}.`;
          return of(null);
        }),
      )
      .subscribe((result) => {
        ...
        this.rows = [
          ...this.rows,
          ...result.items.map((item) => this.toRow(item)),
        ];
        this.hasMore = !!result.nextCursor || result.hasNextPage;
        this.changeDetector.detectChanges();
      });
  ...
  toRow(
    item: MobileClient | MobileUser | MobileRole | MobileConsent,
  ): MobileRow {
    if (this.resource === "clients") {
      const value = item as MobileClient;
      return {
    ...
    if (this.resource === "users") {
      const value = item as MobileUser;
      return {
        id: value.id,
    ...
    if (this.resource === "roles") {
      const value = item as MobileRole;
      return {
        id: value.id,
        name: value.name,
    ...
    const value = item as MobileConsent;
    return {
      id: value.id,
  ...
  bulk(request: HisHopeBulkActionRequest): void {
    if (this.resource === "consents") return;
    this.loading = true;
    this.api
      .bulk(this.resource, request.actionId, request.rowKeys)
      .pipe(
  ...
  exportRows(request: HisHopeTableExportRequest): void {
    void request;
    this.error = "Export is available from the desktop admin workspace.";
  }
}
