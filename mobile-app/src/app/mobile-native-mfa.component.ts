...
@Component({
  ...
  template: `
    @if (status() === "loading") {
      <hh-state
        kind="loading"
        [message]="'mobile.approveSignIn' | hhTranslate"
      />
    } @else if (status() === "approved") {
      <hh-state
        kind="empty"
        [message]="'mobile.signInApproved' | hhTranslate"
      />
    } @else if (status() === "rejected") {
      <hh-state
        kind="error"
        [message]="'mobile.nativeApprovalRejected' | hhTranslate"
      />
    } @else if (status() === "expired") {
      <hh-state
        kind="error"
        [message]="'mobile.nativeApprovalExpired' | hhTranslate"
      />
    } @else if (status() === "retry") {
      <hh-state
        kind="error"
        [message]="'mobile.nativeApprovalRetry' | hhTranslate"
      />
    } @else {
      <hh-state
        kind="error"
        [message]="'mobile.nativeApprovalFailed' | hhTranslate"
      />
    }
  `,
})
export class MobileNativeMfaComponent implements OnInit {
  ...
  readonly status = signal<
    "loading" | "approved" | "rejected" | "expired" | "retry" | "error"
  >("loading");
