...
describe('HisHopeResourceStore', () => {
  it('loads and reloads data for the current query', () => {
    const store = new HisHopeResourceStore<string, string>((query: string) => of(query.toUpperCase()), 'one');
    store.load();
  ...
  it('captures loader failures and releases subscriptions on destroy', () => {
    ...
    const store = new HisHopeResourceStore<string | undefined, string>(() => throwError(() => failure), undefined);
    store.load();
    expect(store.error()).toBe(failure);
    expect(store.loading()).toBeFalse();
    const inFlightStore = new HisHopeResourceStore<string | undefined, string>(() => source, undefined);
    inFlightStore.load();
