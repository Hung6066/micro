...
@Component({
  ...
  template: `
    ...
    >
      @if (searchPlaceholder()) {
        <label class="hh-filter-toolbar__search">
          ...
          <input
            ...
            [attr.aria-label]="searchLabel() | hhTranslate"
            (input)="onSearch($event)"
          />
          @if (query) {
            <button
              type="button"
              class="hh-filter-toolbar__clear"
              (click)="clearSearch()"
      ...
      @if (resultCount() !== null) {
        <span class="hh-filter-toolbar__count"
          >{{ resultCount() }} {{ "common.results" | hhTranslate }}</span
        >
      }
    </section>
  `,
  styles: [
    `
      ...
      .hh-filter-toolbar__search .material-icons {
        color: var(--text-muted);
        font-size: 18px;
      }
      .hh-filter-toolbar__search input {
      ...
      .hh-filter-toolbar__clear:hover {
