...
@Component({
  ...
  providers: [
    {
      ...
      useExisting: forwardRef(() => HisHopeTableEditorComponent),
      multi: true,
    },
  ],
  template: `
    @if (type() === "select") {
      <select
        class="hh-data-table__edit-input"
        [value]="displayValue()"
        [disabled]="disabled"
        (change)="changed($event)"
        (blur)="touched()"
        [attr.aria-label]="ariaLabel() | hhTranslate"
      ...
      <input
        ...
        (input)="changed($event)"
        (blur)="touched()"
        [attr.list]="type() === 'autocomplete' ? listId() : null"
        [attr.aria-label]="ariaLabel() | hhTranslate"
        [attr.aria-invalid]="invalid() ? 'true' : null"
      />
  ...
  styles: [
    `
      ...
      .hh-data-table__edit-input:disabled {
...
export class HisHopeTableEditorComponent implements ControlValueAccessor {
  ...
  displayValue(): string {
    return this.cvaValue === null
      ? String(this.value() ?? "")
      : String(this.cvaValue);
  }
  writeValue(value: unknown): void {
    this.cvaValue = toHisHopeTableEditorValue(value, this.type());
  }
  registerOnChange(fn: (value: HisHopeTableEditorValue) => void): void {
    this.onChange = fn;
  }
  ...
  changed(event: Event): void {
    this.cvaValue = toHisHopeTableEditorValue(
      (event.target as HTMLInputElement | HTMLSelectElement).value,
      this.type(),
    );
    this.onChange(this.cvaValue);
    this.onTouched();
    this.valueChange.emit(this.cvaValue);
  }
}

export type HisHopeTableEditorValue = string | number | null;

export function toHisHopeTableEditorValue(
  value: unknown,
  type: HisHopeTableEditor,
): HisHopeTableEditorValue {
  if (type === "number") {
