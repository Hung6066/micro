...
} from "./his-hope-form-schema";

@Component({
  standalone: true,
  imports: [HisHopeFormRendererComponent],
  template:
    '<hh-form-renderer [fields]="fields" [form]="form" (submitted)="submitted = $event" />',
})
class HostComponent {
  readonly fields: readonly HisHopeFormFieldSchema<unknown>[] = [
    { key: "name", label: "Name", initialValue: "", required: true },
  ];
  readonly form: FormGroup = createHisHopeFormGroup({
    fields: { name: this.fields[0] },
  });
