...
export interface HisHopeTransferItem {
  id: string;
  label: string;
  disabled?: boolean;
}
...
@Component({
  ...
  template: `<div class="hh-transfer">
    <fieldset>
      <legend>{{ availableLabel() }}</legend>
    ...
    <fieldset>
      ...
      @for (item of chosen(); track item.id) {
...
export class HisHopeTransferListComponent {
  ...
  available(): readonly HisHopeTransferItem[] {
    return this.items().filter((item) => !this.selected().has(item.id));
  }
  chosen(): readonly HisHopeTransferItem[] {
    return this.items().filter((item) => this.selected().has(item.id));
  }
  ...
  moveSelected(): void {
    this.selected.update((current) => new Set(current));
  }
}
