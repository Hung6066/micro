...
export class HisHopeQueryStateService {
  ...
  private readonly current = new BehaviorSubject<HisHopePageQuery>({
    page: 1,
    pageSize: 20,
  });
  ...
  patch(patch: Partial<HisHopePageQuery>): void {
    this.update({ ...this.current.value, ...patch, page: patch.page ?? 1 });
  }
}
