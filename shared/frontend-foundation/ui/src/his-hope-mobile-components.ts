...
@Component({
  ...
  styles: [`:host{display:block}.hh-mobile-infinite-list{display:grid;gap:0}.hh-mobile-infinite-list__loading{display:grid;gap:8px;padding:16px}.hh-mobile-skeleton{display:block;height:12px;border-radius:999px;background:linear-gradient(90deg,var(--skeleton-base),var(--skeleton-highlight),var(--skeleton-base));background-size:200% 100%;animation:hh-mobile-shimmer 1.2s linear infinite}.hh-mobile-skeleton--short{width:62%}.hh-mobile-button{display:block;width:100%;min-height:44px;margin-top:12px;border:1px solid var(--border-default);border-radius:var(--radius-control);background:var(--surface-white);color:var(--color-primary);font:inherit;font-weight:var(--font-weight-semibold)}.hh-mobile-list-end{margin:12px 0;color:var(--text-muted);font-size:12px;text-align:center}@keyframes hh-mobile-shimmer{to{background-position:-200% 0}}`],
})
export class HisHopeMobileInfiniteListComponent {
  readonly label = input('List'); readonly loading = input(false); readonly hasMore = input(false); readonly loadedCount = input(0); readonly totalCount = input<number | null>(null); readonly nextCursor = input(''); readonly loadMoreLabel = input('Load more'); readonly endLabel = input('All records loaded');
  readonly loadMore = output<void>(); readonly loadMoreRequested = output<{ cursor: string | null }>();
  requestLoadMore(): void { this.loadMore.emit(); this.loadMoreRequested.emit({ cursor: this.nextCursor() || null }); }
}
...
export class HisHopeMobileRefresherComponent {
  readonly threshold = input(72); readonly pullLabel = input('Pull to refresh'); readonly releaseLabel = input('Release to refresh'); readonly refreshed = output<void>();
  distance = 0; ready = false; private startY: number | null = null;
  start(event: PointerEvent): void { if (window.scrollY <= 0) this.startY = event.clientY; }
  move(event: PointerEvent): void { if (this.startY === null || window.scrollY > 0) return; this.distance = Math.min(96, Math.max(0, event.clientY - this.startY)); this.ready = this.distance >= this.threshold(); }
  end(): void { const shouldRefresh = this.ready; this.startY = null; this.distance = 0; this.ready = false; if (shouldRefresh) this.refreshed.emit(); }
}
...
export class HisHopeMobileSearchbarComponent {
  readonly value = input(''); readonly placeholder = input('Search'); readonly label = input('Search'); readonly clearLabel = input('Clear search'); readonly valueChange = output<string>();
  private timer: ReturnType<typeof setTimeout> | null = null;
  onInput(value: string): void { if (this.timer) clearTimeout(this.timer); this.timer = setTimeout(() => this.valueChange.emit(value.trim()), 250); }
  clear(input: HTMLInputElement): void { if (this.timer) clearTimeout(this.timer); input.value = ''; this.valueChange.emit(''); input.focus(); }
}
...
export class HisHopeMobileActionSheetComponent implements OnChanges {
  readonly open = input(false); readonly label = input('Actions'); readonly cancelLabel = input('Cancel'); readonly close = output<void>();
  private previousActiveElement: HTMLElement | null = null;
  constructor(private readonly host: ElementRef<HTMLElement>) {}
  ngOnChanges(changes: SimpleChanges): void {
    if (changes['open']?.currentValue) {
      this.previousActiveElement = document.activeElement as HTMLElement | null;
      queueMicrotask(() => focusableElements(this.host.nativeElement)[0]?.focus());
    } else if (changes['open'] && !changes['open'].currentValue) {
      this.previousActiveElement?.focus(); this.previousActiveElement = null;
    }
  }
  @HostListener('document:keydown', ['$event']) onKeydown(event: KeyboardEvent): void {
    ...
    const items = focusableElements(this.host.nativeElement); if (!items.length) return;
...
export class HisHopeMobileBottomSheetComponent implements OnChanges {
  readonly open = input(false); readonly label = input('Details'); readonly closeLabel = input('Close'); readonly close = output<void>();
  private previousActiveElement: HTMLElement | null = null;
  constructor(private readonly host: ElementRef<HTMLElement>) {}
  ngOnChanges(changes: SimpleChanges): void {
    if (changes['open']?.currentValue) {
...
export class HisHopeMobileSegmentComponent {
  readonly label = input('View'); readonly options = input<Array<{ value: string; label: string }>>([]); readonly value = input(''); readonly valueChange = output<string>();
}
...
export class HisHopeMobileAccordionComponent {
  readonly title = input('Section'); open = false;
}
...
export class HisHopeMobileAvatarComponent {
  readonly label = input('Avatar'); readonly src = input(''); readonly initials = input('');
}
...
export class HisHopeMobileDateTimeComponent {
  readonly label = input('Date and time'); readonly mode = input<'date' | 'datetime-local'>('datetime-local'); readonly value = input(''); readonly min = input(''); readonly max = input(''); readonly valueChange = output<string>();
}
...
export class HisHopeMobileOtpComponent {
  readonly label = input('Verification code'); readonly length = input(6); readonly valueChange = output<string>(); readonly completed = output<string>(); digits: string[] = [];
  @ViewChildren('otpInput') private inputs!: QueryList<ElementRef<HTMLInputElement>>;
  constructor() { this.digits = Array.from({ length: this.length() }, () => ''); }
  update(index: number, value: string): void { const digit = value.replace(/\D/g, '').slice(-1); this.digits[index] = digit; this.valueChange.emit(this.digits.join('')); if (digit && index < this.digits.length - 1) this.inputs?.get(index + 1)?.nativeElement.focus(); if (this.digits.every(Boolean)) this.completed.emit(this.digits.join('')); }
  backspace(index: number): void { if (!this.digits[index] && index > 0) this.inputs?.get(index - 1)?.nativeElement.focus(); }
  paste(event: ClipboardEvent): void { event.preventDefault(); const value = event.clipboardData?.getData('text')?.replace(/\D/g, '').slice(0, this.digits.length) ?? ''; [...value].forEach((digit, index) => this.digits[index] = digit); this.valueChange.emit(this.digits.join('')); if (this.digits.every(Boolean)) this.completed.emit(this.digits.join('')); }
}
