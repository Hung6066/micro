...
import {
  ...
  HisHopeBulkJob,
  HisHopeConflictResolution,
  HisHopeFilterGroup,
  HisHopeFilterOperator,
  HisHopePageQuery,
  HisHopeSort,
  HisHopeTableAnalysisRequest,
  HisHopeTableEditConflict,
  HisHopeTableEditSaveRequest,
  HisHopeTableEditState,
  HisHopeTableEditor,
  HisHopeTableEditorOption,
...
export function serializeHisHopeDataTableQuery(
  query: HisHopePageQuery,
): HisHopeDataTableUrlParams {
  return {
 ...
 */
export function sameHisHopeDataTableQuery(
  left: HisHopePageQuery,
  right: HisHopePageQuery,
): boolean {
  return (
    JSON.stringify(serializeHisHopeDataTableQuery(left)) ===
    JSON.stringify(serializeHisHopeDataTableQuery(right))
  );
}

export function parseHisHopeDataTableQuery(
  ...
  const query: HisHopePageQuery = {
    page: Number.isFinite(page) && page > 0 ? page : 1,
    pageSize: Number.isFinite(pageSize) && pageSize > 0 ? pageSize : 20,
  };
  const cursor = read("cursor");
  const search = read("search")?.trim();
  const sort = parseJson<HisHopeSort>("sort");
  const filters = parseJson<HisHopePageQuery["filters"]>("filters");
  const filterItems = parseJson<HisHopePageQuery["filterItems"]>("filterItems");
  if (cursor) query.cursor = cursor;
  if (search) query.search = search;
  if (sort) query.sort = sort;
  if (filters) query.filters = filters;
  if (filterItems) query.filterItems = filterItems;
  return query;
}

export function toggleHisHopeDataTableSort(
  current: HisHopeDataTableSort[],
  key: string,
  append: boolean,
): HisHopeDataTableSort[] {
  const existing = current.find((item) => item.key === key);
  const nextTerm: HisHopeDataTableSort = {
    key,
    direction: existing?.direction === "asc" ? "desc" : "asc",
  };
  return append
    ? [...current.filter((item) => item.key !== key), nextTerm]
    : [nextTerm];
}

export function createHisHopeCursorQuery(
  query: HisHopePageQuery,
  cursor: string | null,
): HisHopePageQuery {
...
@Component({
  ...
  template: `
    <section
      class="hh-data-table"
      [class.hh-data-table--compact]="density() === 'compact'"
      [class.hh-data-table--mobile-list]="mobilePresentation() !== 'table'"
      [class.hh-data-table--sticky-header]="stickyHeader()"
      [class.hh-data-table--sticky-footer]="stickyFooter()"
    ...
    >
      @if (loading() && !rows().length) {
        <div class="hh-data-table__state" role="status" aria-live="polite">
          <div class="hh-data-table__skeleton-list" aria-hidden="true">
            @for (item of [1, 2, 3, 4, 5]; track item) {
              <div class="hh-data-table__skeleton-item">
        ...
        <div
          class="hh-data-table__state hh-data-table__state--error"
          role="alert"
        >
          ...
          <button
            ...
            (click)="retry.emit()"
          >
            {{ retryLabel() | hhTranslate }}
          </button>
        </div>
      } @else if (configuredColumns().length > 0) {
        @if (error() && rows().length) {
          <div class="hh-data-table__stale-error" role="alert">
            <span class="material-icons" aria-hidden="true">error_outline</span
            ><span>{{ error() | hhTranslate }}</span
            ><button
              type="button"
        ...
        @if (loading()) {
          <div
            class="hh-data-table__loading-overlay"
            role="status"
            aria-live="polite"
          >
            <span class="hh-spinner" aria-hidden="true"></span
            >{{ loadingMessage() | hhTranslate }}
          </div>
        }
        <div class="hh-data-table__toolbar">
          @if (searchable()) {
            <div class="hh-data-table__search" role="search">
              ...
              <input
                type="search"
                [value]="searchValue()"
                [placeholder]="searchPlaceholder() | hhTranslate"
                [attr.aria-label]="searchPlaceholder() | hhTranslate"
              ...
              @if (searchValue()) {
                <button
                  ...
                  [attr.aria-label]="'common.clearSearch' | hhTranslate"
                  [attr.title]="'common.clearSearch' | hhTranslate"
                  (click)="clearSearch()"
                >
          ...
          <span class="hh-data-table__selection" aria-live="polite"
            >{{ selectionCount() }} selected</span
          >
          @if (bulkJob(); as job) {
            <span class="hh-data-table__bulk-job" role="status"
              >{{ job.status }} {{ job.processed }}/{{ job.total }}</span
            >
            @if (job.rowProgress?.length) {
              <span class="hh-data-table__bulk-row-progress"
                >{{ completedRows(job) }} rows completed</span
              >
            }
            @if (job.status === "queued" || job.status === "running") {
              <button
                type="button"
                class="hh-button hh-button--secondary hh-button--small"
                (click)="bulkJobCancelled.emit(job.jobId)"
              >
                Cancel job
              </button>
            }
            @if (job.status === "failed" && job.retryable) {
              <button
                type="button"
                class="hh-button hh-button--secondary hh-button--small"
                (click)="bulkJobRetry.emit(job.jobId)"
              >
                Retry job
              </button>
            }
          ...
          @if (exportable()) {
            <label class="hh-data-table__export"
              >Export as
              <select
                [value]="exportFormat()"
                (change)="changeExportFormat($event)"
                [attr.aria-label]="'common.exportFormat' | hhTranslate"
              >
            ...
            <button
              ...
              (click)="requestExport()"
            >
              {{ "table.export" | hhTranslate }}
            </button>
            @if (exportJob(); as job) {
              <span class="hh-data-table__bulk-job" role="status"
                >Export {{ job.status }} {{ job.processed }}/{{
                  job.total
                }}</span
              >
              @if (job.status === "queued" || job.status === "running") {
          ...
          @if (filterBuilder()) {
            <details class="hh-data-table__filters">
              ...
              >
                ...
                <button
                  type="button"
                  class="hh-button hh-button--primary hh-button--small"
                  (click)="applyFilters()"
                >
                  Apply filters
                </button>
          ...
          @if (
            ...
            >
              ...
              @for (operation of analysisOperations(); track operation) {
                <button
                  type="button"
                  class="hh-button hh-button--secondary hh-button--small"
                  (click)="requestAnalysis(operation)"
                >
                  {{ operation }}
                </button>
              }
          ...
          @if (hasSelection() && bulkActions().length) {
            ...
            >
              @for (action of bulkActions(); track action.id) {
                <button
                  type="button"
                  [class.hh-button]="!action.icon"
                  [class.hh-button--secondary]="
                    !action.icon && action.tone !== 'danger'
                  "
                  [class.hh-button--danger]="
                    action.tone === 'danger' && !action.icon
                  "
                  [class.hh-button--small]="!action.icon"
                  [class.hh-icon-button]="!!action.icon"
                  [class.hh-icon-button--danger]="
                    !!action.icon && action.tone === 'danger'
                  "
                  [disabled]="action.disabled || !canRunAction(action)"
                  [hidden]="!canRunAction(action)"
                  [attr.aria-label]="action.label"
                  [attr.title]="action.icon ? action.label : null"
        ...
        @if (!rows().length) {
          ...
          <div
            class="hh-data-table__content"
            [class.hh-data-table__content--virtualized]="virtualize()"
            (scroll)="onVirtualScroll($event)"
          >
            <table>
              <caption class="hh-visually-hidden">
                {{
                  label() | hhTranslate
                }}
              </caption>
              <thead>
                <tr>
                  @if (selection()) {
                    <th scope="col" class="hh-data-table__select">
                      <input
                        type="checkbox"
                        [checked]="allVisibleSelected()"
                        (change)="toggleAll()"
                        [attr.aria-label]="'common.selectAllRows' | hhTranslate"
                      />
                    </th>
                  }
                  @if (virtualizeColumns() && columnVirtualLeadingWidth() > 0) {
                    <th
                      class="hh-data-table__column-spacer"
                      [style.width.px]="columnVirtualLeadingWidth()"
                      aria-hidden="true"
                    ></th>
                  }
                  @for (column of renderedColumns(); track column.key) {
                    <th
                      ...
                      [class.hh-data-table__actions-column]="
                        column.key === 'actions'
                      "
                      [class.hh-data-table__cell--pin-left]="
                        column.pinned === 'left'
                      "
                      [class.hh-data-table__cell--pin-right]="
                        column.pinned === 'right'
                      "
                      [style.left.px]="pinnedOffset(column, 'left')"
                      [style.right.px]="pinnedOffset(column, 'right')"
                      [attr.draggable]="column.reorderable !== false"
                      (dragstart)="beginColumnDrag(column.key)"
                    ...
                    >
                      @if (column.sortable) {
                        <button
                          type="button"
                          class="hh-data-table__sort"
                          (click)="sortBy(column.key, $event)"
                          [attr.aria-label]="sortLabel(column)"
                          [attr.aria-pressed]="sortStates().some(item => item.key === column.key)"
                        >
                          {{ column.label }}
                          <span
                            class="material-icons hh-data-table__sort-icon"
                            aria-hidden="true"
                            >{{ sortIcon(column.key) }}</span
                          >
                          @if (sortPriority(column.key); as priority) {
                            <span class="hh-data-table__sort-priority">{{
                              priority
                            }}</span>
                          }
                        </button>
                      ...
                      @if (column.reorderable !== false) {
                        <button
                          ...
                          "
                          (keydown.arrowleft)="
                            moveColumn(column.key, -1); $event.preventDefault()
                          "
                          (keydown.arrowright)="
                            moveColumn(column.key, 1); $event.preventDefault()
                          "
                        ...
                        <button
                          type="button"
                          class="hh-data-table__resize"
                          role="separator"
                          aria-orientation="vertical"
                          [attr.aria-label]="
                            'Resize ' + column.label + ' column'
                          "
                  ...
                  @if (
                    virtualizeColumns() && columnVirtualTrailingWidth() > 0
                  ) {
                    <th
                      class="hh-data-table__column-spacer"
                      [style.width.px]="columnVirtualTrailingWidth()"
                      aria-hidden="true"
                    ></th>
                  }
                  @if (inlineEdit()) {
                    <th scope="col" class="hh-data-table__edit-actions">
              ...
              <tbody>
                  ...
                  <tr aria-hidden="true">
                    <td
                      [attr.colspan]="
                        ...
                        (inlineEdit() ? 1 : 0)
                      "
                      [style.height.px]="virtualStart() * virtualRowHeight()"
                    ></td>
                  </tr>
                }
                @for (
                  row of renderedRows();
                  track rowKeyValue(row);
                  let rowIndex = $index
                ) {
                  <tr
                    [class.hh-data-table__row--clickable]="rowClickable()"
                    ...
                    [attr.aria-label]="
                      rowClickable() ? 'Open row ' + rowKeyValue(row) : null
                    "
                  >
                    @if (selection()) {
                      <td class="hh-data-table__select">
                        <input
                          type="checkbox"
                    ...
                    @if (
                      virtualizeColumns() && columnVirtualLeadingWidth() > 0
                    ) {
                      <td
                        class="hh-data-table__column-spacer"
                        [style.width.px]="columnVirtualLeadingWidth()"
                    ...
                    @for (
                      ...
                      >
                        @if (
                          columnIndex === 0 &&
                          (detailTemplate() || treeChildrenKey())
                        ) {
                          <button
                            type="button"
                            ...
                            "
                            (click)="
                              toggleExpanded(row); $event.stopPropagation()
                            "
                          >
                            <span class="material-icons" aria-hidden="true">{{
                        ...
                        @if (isEditing(row) && column.editable) {
                          <hh-table-editor
                            ...
                            [invalid]="!!editError(column.key)"
                            [listId]="'hh-options-' + column.key"
                            (valueChange)="updateDraftValue(column.key, $event)"
                          />
                          @if (editError(column.key); as message) {
                            <small
                              class="hh-data-table__edit-error"
                              role="alert"
                              >{{ message }}</small
                            >
                          }
                          ...
                          @if (cellTemplate(column.key); as template) {
                            <span
                              class="hh-data-table__cell-template"
                              [class.hh-data-table__actions-content]="
                                column.key === 'actions'
                              "
                              ><ng-container
                                *ngTemplateOutlet="
                    ...
                    @if (inlineEdit()) {
                      <td class="hh-data-table__edit-actions">
                        <div class="hh-data-table__edit-actions-content">
                          @if (isEditing(row)) {
                            @if (isSaving(row)) {
                              <span class="hh-data-table__saving" role="status"
                                >Saving...</span
                              >
                            }
                            @if (editErrorMessage(); as message) {
                              <small
                                class="hh-data-table__save-error"
                                role="alert"
                                >{{ message }}</small
                              >
                            }
                            @if (editErrorMessage()) {
                              <button
                                ...
                                class="hh-button hh-button--secondary hh-button--small"
                                (click)="
                                  retryEditSave(); $event.stopPropagation()
                                "
                              >
                                {{ "common.retry" | hhTranslate }}
                              </button>
                            ...
                            @if (editConflict(); as conflict) {
                              <small
                                class="hh-data-table__save-error"
                                role="alert"
                                >{{ conflict.message }}</small
                              >
                              <button
                                type="button"
                                class="hh-button hh-button--secondary hh-button--small"
                                (click)="
                                  resolveConflict('latest');
                                  $event.stopPropagation()
                                "
                              >
                                {{ "common.useLatest" | hhTranslate }}
                              </button>
                              <button
                                type="button"
                                class="hh-button hh-button--secondary hh-button--small"
                                ...
                                "
                              >
                                {{ "common.keepMine" | hhTranslate }}
                              </button>
                              @if (conflict.mergeableFields?.length) {
                                <button
                                  type="button"
                                  class="hh-button hh-button--secondary hh-button--small"
                                  (click)="
                                  ...
                                  "
                                >
                                  {{ "common.merge" | hhTranslate }}
                                </button>
                              }
                            }
                            <span
                              class="hh-data-table__dirty"
                              [class.hh-data-table__dirty--visible]="
                                isDirty(row)
                              "
                              >{{ isDirty(row) ? "Unsaved changes" : "" }}</span
                            >
                            <button
                              type="button"
                              class="hh-button hh-button--primary hh-button--small"
                              [disabled]="hasEditErrors() || isSaving(row)"
                              (click)="saveEdit(row); $event.stopPropagation()"
                            >
                              {{ "common.save" | hhTranslate }}
                            </button>
                            <button
                              type="button"
                              class="hh-button hh-button--secondary hh-button--small"
                            ...
                            @if (lastEdit(); as transaction) {
                              <button
                                type="button"
                                class="hh-button hh-button--secondary hh-button--small"
                                (click)="
                                  undoEdit(transaction);
                                  $event.stopPropagation()
                                "
                              >
                                {{ "common.undo" | hhTranslate }}
                              </button>
                            }
                          } @else {
                            <hh-action-button
                  ...
                  @if (isExpanded(row) && detailTemplate()) {
                    <tr class="hh-data-table__detail-row">
                      <td
                        [attr.colspan]="
                          visibleColumns().length +
                          (selection() ? 1 : 0) +
                ...
                @if (virtualize() && virtualEnd() < pagedRows().length) {
                  <tr aria-hidden="true">
                    <td
                      [attr.colspan]="
                        visibleColumns().length +
                      ...
                      "
                      [style.height.px]="
                        (pagedRows().length - virtualEnd()) * virtualRowHeight()
                      "
                    ></td>
                  </tr>
                }
          ...
          >
            @for (row of renderedRows(); track rowKeyValue(row)) {
              <article
                class="hh-data-table__mobile-item"
                [class.hh-data-table__row--clickable]="
                ...
                (keydown.enter)="onMobileRowClick(row)"
              ...
              >
                        ...
                        <dd>
                          @if (isEditing(row) && column.editable) {
                            <hh-table-editor
                              ...
                              [ariaLabel]="'Edit ' + column.label"
                              [invalid]="!!editError(column.key)"
                              [listId]="'hh-mobile-options-' + column.key"
                              (valueChange)="
                                updateDraftValue(column.key, $event)
                              "
                            />
                            @if (editError(column.key); as message) {
                              <small
                            ...
                            <hh-status-badge
                              [status]="displayValue(row, column.key)"
                              [label]="displayValue(row, column.key)"
                            />
                          } @else {
                            {{ cellValue(row, column.key) }}
                          }
                ...
                @if (detailTemplate() && isExpanded(row)) {
                  <div class="hh-data-table__mobile-detail" role="region">
                    <ng-container
                      *ngTemplateOutlet="
                        detailTemplate()!.template;
                        context: { $implicit: row, row: row }
          ...
          @if (mobileLoadMore()) {
            <section class="hh-data-table__mobile-load-more" aria-live="polite">
              ...
              @if (mobileLoadMoreError()) {
                <span
                  class="hh-data-table__mobile-load-more-error"
                  role="alert"
                  >{{ mobileLoadMoreError() | hhTranslate }}</span
                >
                <button
                  type="button"
                  class="hh-button hh-button--secondary hh-button--small"
                  [disabled]="mobileLoadingMore()"
                  (click)="mobileLoadMoreRequested.emit()"
                >
                  {{ "common.retry" | hhTranslate }}
                </button>
                ...
                <button
                  ...
                  (click)="mobileLoadMoreRequested.emit()"
                >
                  {{ "common.loadMore" | hhTranslate }}
                </button>
              } @else if (mobileLoadedCount() > 0) {
                <span class="hh-data-table__mobile-load-more-status"
                  >All records loaded</span
                >
              }
            </section>
          }
          <nav class="hh-data-table__pagination" aria-label="Table pagination">
            @if (cursorPaging()) {
              <button
                ...
                (click)="changeCursor(previousCursor())"
                [attr.aria-label]="'common.previousPage' | hhTranslate"
              >
                {{ "common.previousPage" | hhTranslate }}
              </button>
              ...
              <button
                ...
                class="hh-button hh-button--secondary"
                [disabled]="!nextCursor()"
                (click)="changeCursor(nextCursor())"
                [attr.aria-label]="'common.nextPage' | hhTranslate"
              >
                {{ "common.nextPage" | hhTranslate }}
              </button>
            } @else {
              <label class="hh-data-table__page-size"
                >Rows
                <select
                  [value]="pageSizeValue()"
              ...
              >
              <span
                >{{ pageStart() }}-{{ pageEnd() }} of {{ totalRows() }}</span
              >
              <button
                type="button"
                class="hh-button hh-button--secondary"
              ...
              <span aria-current="page"
                >Page {{ pageValue() }} of {{ pageCount() }}</span
              >
              <button
                type="button"
                class="hh-button hh-button--secondary"
  ...
  styles: [
    `
      ...
      .hh-data-table__loading-overlay {
        position: absolute;
        inset: 0;
        z-index: 3;
        display: flex;
      ...
      .hh-data-table__stale-error span:nth-child(2) {
        flex: 1;
      }
      .hh-data-table__content {
        position: relative;
        z-index: 1;
        min-width: 0;
        overflow-x: auto;
      }
      .hh-data-table__content--virtualized {
        max-height: 560px;
        overflow: auto;
      }
      .hh-data-table__mobile-list {
        display: none;
      }
      ...
      .hh-data-table__mobile-item:last-child {
        border-bottom: 0;
      }
      ...
      .hh-data-table__mobile-fields dd {
        min-width: 0;
        margin: 0;
      ...
      .hh-data-table__mobile-fields dd hh-status-badge {
        display: inline-flex;
      }
      ...
      .hh-data-table__row--clickable {
        cursor: pointer;
      }
      .hh-data-table__row--clickable:hover {
      ...
      .hh-data-table__search:focus-within {
      ...
      .hh-data-table__search input[type="search"]::-webkit-search-decoration {
        -webkit-appearance: none;
        appearance: none;
      }
      .hh-data-table__search input[type="search"]::-ms-clear {
        display: none;
        width: 0;
        height: 0;
      }
      .hh-data-table__search input::placeholder {
      ...
      .hh-data-table__sort:hover,
      .hh-data-table__sort:focus-visible {
        background: var(--surface-hover);
      }
      ...
      .hh-data-table__sort:focus-visible .hh-data-table__sort-icon {
        color: var(--color-primary);
      }
      .hh-data-table__sort-priority {
        display: inline-grid;
        place-items: center;
        min-width: 14px;
        height: 14px;
      ...
      .hh-data-table__edit-actions-content {
        display: flex;
        flex-wrap: wrap;
        align-items: center;
      ...
      .hh-data-table__edit-input[aria-invalid="true"] {
      ...
      .hh-data-table__saving {
        color: var(--text-secondary);
        font-size: var(--font-size-caption);
      }
      .hh-data-table__save-error {
        color: var(--color-danger);
        font-size: var(--font-size-caption);
      }
      .hh-data-table__dirty {
        display: none;
        color: var(--color-warning);
        font-size: var(--font-size-caption);
      }
      .hh-data-table__dirty--visible {
        display: inline;
      }
      ...
      .hh-data-table__columns summary {
        cursor: pointer;
        color: var(--text-secondary);
        font-size: var(--font-size-caption);
      }
      ...
      .hh-data-table__filters {
        position: relative;
        z-index: 20;
      }
      .hh-data-table__filters > summary {
        display: inline-flex;
        align-items: center;
        justify-content: center;
        gap: 4px;
      ...
      .hh-data-table__filters > summary::-webkit-details-marker {
        display: none;
      }
      ...
      .hh-data-table__cell--pin-left,
      .hh-data-table__cell--pin-right {
        position: sticky;
        z-index: 2;
        background: var(--surface-white);
        box-shadow: 1px 0 0 var(--border-light);
      ...
      .hh-data-table--sticky-header thead th {
        position: sticky;
        top: 0;
        z-index: 3;
        background: var(--surface-white);
      }
      .hh-data-table--sticky-header thead th.hh-data-table__cell--pin-left,
      .hh-data-table--sticky-header thead th.hh-data-table__cell--pin-right {
        z-index: 4;
      }
      .hh-data-table--sticky-footer .hh-data-table__pagination {
        position: sticky;
        bottom: 0;
        z-index: 3;
        background: var(--surface-white);
      ...
      .hh-data-table__mobile-load-more-error {
        color: var(--color-danger);
      }
      ...
      .hh-data-table__mobile-load-more-skeleton span:last-child {
        flex: 0.55;
      }
      ...
      .hh-data-table__skeleton-item:first-child {
        border-top: 1px solid var(--border-light);
      }
      .hh-data-table__skeleton-item span {
        display: block;
        width: 100%;
        height: 12px;
        border-radius: 6px;
      ...
      .hh-data-table__skeleton-item span:nth-child(4) {
        width: 70%;
      }
      .hh-data-table__skeleton-item span:last-child {
        width: 58%;
        justify-self: end;
      }
      ...
      @media (max-width: 768px) {
        ...
        .hh-data-table--mobile-list .hh-data-table__mobile-load-more {
          display: flex;
        }
        .hh-data-table--mobile-list .hh-data-table__pagination {
          display: none;
        }
        .hh-data-table__columns-menu {
          right: auto;
      ...
      @media (max-width: 600px) {
        .hh-data-table--mobile-list .hh-data-table__content {
          display: none;
        }
        .hh-data-table--mobile-list .hh-data-table__mobile-list {
          display: grid;
        }
        .hh-data-table__mobile-fields > div {
          grid-template-columns: 1fr;
...
export class HisHopeDataTableComponent {
  ...
  readonly mobilePresentation =
    input<HisHopeDataTableMobilePresentation>("auto");
  ...
  readonly savedView = input<{
    hiddenColumns?: string[];
    columnOrder?: string[];
    columnWidths?: Record<string, number>;
    sort?: HisHopeSort | null;
  } | null>(null);
  readonly permissionChecker = input<(permission?: string) => boolean>(
    () => true,
  );
  ...
  @ContentChildren(HisHopeDataTableCellDirective)
  readonly cellTemplates!: QueryList<HisHopeDataTableCellDirective>;
  @ContentChildren(HisHopeDataTableDetailDirective)
  readonly detailTemplates!: QueryList<HisHopeDataTableDetailDirective>;
  ...
  >();
  @Output() readonly selectionStateChange =
    new EventEmitter<HisHopeTableSelectionState>();
  ...
  @Output() readonly exportRequested =
    new EventEmitter<HisHopeTableExportRequest>();
  @Output() readonly bulkActionRequested =
    new EventEmitter<HisHopeBulkActionRequest>();
  ...
  @Output() readonly columnResizeChange = new EventEmitter<{
    key: string;
    width: number;
  }>();
  @Output() readonly rowEditSaveRequested =
    new EventEmitter<HisHopeTableEditSaveRequest>();
  @Output() readonly rowEditUndo = new EventEmitter<{
    previous: Record<string, unknown>;
    current: Record<string, unknown>;
  }>();
  @Output() readonly rowEditConflict =
    new EventEmitter<HisHopeTableEditConflict>();
  @Output() readonly conflictResolved = new EventEmitter<{
    resolution: HisHopeConflictResolution;
    conflict: HisHopeTableEditConflict;
  }>();
  ...
  @Output() readonly rowExpandChange = new EventEmitter<{
    rowKey: string;
    expanded: boolean;
  }>();
  @Output() readonly importRequested =
    new EventEmitter<HisHopeTableImportRequest>();
  @Output() readonly analysisRequested =
    new EventEmitter<HisHopeTableAnalysisRequest>();
  ...
  @Output() readonly viewSaveRequested = new EventEmitter<{
    name: string;
    payload: {
      hiddenColumns: string[];
      columnOrder: string[];
  ...
  readonly filterOperators: HisHopeFilterOperator[] = [
    "eq",
    "neq",
    "contains",
    "startsWith",
  ...
  constructor() {
    ...
    effect(() => {
      ...
      if (incoming.length && !this.localFilterGroups().length)
        this.localFilterGroups.set(incoming);
    });
    effect(() => {
    ...
    effect(() => {
      ...
      if (!incoming) return;
      this.selectAllPages =
        incoming.scope === "all" && incoming.selectedKeys.length === 0;
    ...
    this.route?.queryParams
      ...
      .subscribe((params) => {
        ...
        this.urlReady = true;
        if (!sameHisHopeDataTableQuery(this.query(), next))
    ...
    effect(() => {
      ...
      void this.router.navigate([], {
        ...
        queryParams: serializeHisHopeDataTableQuery(query),
  ...
  @HostListener("window:resize") onViewportResize(): void {
    if (typeof window !== "undefined")
      this.viewportWidth.set(window.innerWidth);
  }
  ...
  visibleColumns(): HisHopeDataTableColumn[] {
    const hidden = this.localHiddenColumns().length
      ? this.localHiddenColumns()
      : this.hiddenColumns();
    const responsiveVisible = this.responsiveVisibleColumns();
    const order = this.localColumnOrder().length
      ? this.localColumnOrder()
      : this.columnOrder();
    const columns = order.length
      ? [...this.columns()].sort(
          (a, b) =>
            this.orderIndex(order, a.key) - this.orderIndex(order, b.key),
        )
      : this.columns();
    return columns.filter(
      (column) =>
        !hidden.includes(column.key) &&
        (!this.isResponsiveHidden(column) ||
          responsiveVisible.includes(column.key)),
    );
  }
  renderedColumns(): HisHopeDataTableColumn[] {
    ...
    const slots = Math.max(
      1,
      Math.ceil(this.columnVirtualViewport() / width) + 2,
    );
    const start = Math.min(
      this.columnVirtualStart(),
      Math.max(0, middle.length - slots),
    );
    return [...left, ...middle.slice(start, start + slots), ...right];
  }
  columnVirtualLeadingWidth(): number {
    if (!this.virtualizeColumns() || this.visibleColumns().length < 12)
      return 0;
    const middle = this.visibleColumns().filter((column) => !column.pinned);
    const left = this.visibleColumns().filter(
    ...
    const start = Math.min(
      ...
      Math.max(
        0,
        middle.length - (Math.ceil(this.columnVirtualViewport() / 160) + 2),
      ),
    );
  ...
  columnVirtualTrailingWidth(): number {
    ...
    const slots = Math.max(
      1,
      Math.ceil(this.columnVirtualViewport() / 160) + 2,
    );
    const start = Math.min(
      this.columnVirtualStart(),
      Math.max(0, middle.length - slots),
  ...
  mobileTitleKey(): string {
    return this.visibleColumns()[0]?.key ?? "";
  }
  mobileItemTitle(row: Record<string, unknown>): string {
    return this.cellValue(row, this.mobileTitleKey()) || this.rowKeyValue(row);
  }
  private orderIndex(order: string[], key: string): number {
    return orderIndexOf(order, key);
  }
  columnWidth(column: HisHopeDataTableColumn): number | null {
    const width = this.localColumnWidths()[column.key];
    if (width) return width;
    const parsed = column.width ? Number.parseFloat(column.width) : NaN;
    return Number.isFinite(parsed) ? parsed : null;
  }
  pinnedOffset(
    column: HisHopeDataTableColumn,
    side: "left" | "right",
  ): number | null {
    if (column.pinned !== side) return null;
    const columns = this.visibleColumns().filter(
      (item) => item.pinned === side,
    );
    const index = columns.findIndex((item) => item.key === column.key);
    if (index < 0) return null;
    return columns
      .slice(0, index)
      .reduce((total, item) => total + (this.columnWidth(item) ?? 160), 0);
  }
  filterCount(): number {
    return this.localFilterGroups().reduce(
      (count, group) =>
        count +
  ...
  operatorLabel(operator: HisHopeFilterOperator): string {
    return operatorLabelPure(operator);
  }
  addFilterGroup(): void {
    this.localFilterGroups.update((groups) => [
      ...groups,
      {
        join: "and",
        filters: [
          {
            key: this.configuredColumns()[0]?.key ?? "",
            operator: "contains",
            value: "",
          },
        ],
    ...
    this.localFilterGroups.update((groups) =>
      groups.map((group, index) =>
        index !== groupIndex
          ...
          : {
              ...group,
              filters: group.filters.map((filter, current) =>
                current !== filterIndex
                  ? filter
                  : {
                      ...filter,
                      [field]:
                        field === "operator"
                          ? (value as HisHopeFilterOperator)
                          : value,
                    },
              ),
            },
  ...
  applyFilters(): void {
    this.filterChange.emit(this.localFilterGroups());
    this.queryChange.emit({
      ...this.query(),
      page: 1,
      cursor: undefined,
      filters: Object.fromEntries(
        this.localFilterGroups().flatMap((group) =>
          group.filters
  ...
  canRunAction(action: HisHopeBulkAction): boolean {
    return this.permissionChecker()(action.permission);
  }
  saveView(): void {
    const view = {
      hiddenColumns: this.localHiddenColumns(),
      columnOrder: this.localColumnOrder(),
      columnWidths: this.localColumnWidths(),
      sort: this.sort() ?? this.localSort(),
    };
  ...
  resetView(): void {
    ...
    if (this.serverBackedView())
      this.viewResetRequested.emit({ name: this.viewName() });
    this.localHiddenColumns.set([]);
    this.localColumnOrder.set([]);
    this.localColumnWidths.set({});
    this.localSort.set(null);
  ...
  private restoreSavedView(key: string): void {
    ...
    try {
      ...
      /* Ignore a corrupted preference and keep the default view. */
    }
  }
  private applySavedView(saved: {
    hiddenColumns?: string[];
    columnOrder?: string[];
    columnWidths?: Record<string, number>;
    sort?: HisHopeSort | null;
  }): void {
    this.localHiddenColumns.set(saved.hiddenColumns ?? []);
    this.localColumnOrder.set(saved.columnOrder ?? []);
    this.localColumnWidths.set(saved.columnWidths ?? {});
    this.localSort.set(saved.sort ?? null);
  }
  onCellKeydown(
    event: KeyboardEvent,
    rowIndex: number,
    ...
    const nextCell = nextRow
      ? (Array.from(nextRow.querySelectorAll("td"))[columnIndex + delta[1]] as
          | HTMLElement
          | undefined)
      : undefined;
    if (nextCell) {
  ...
  copyTable(): void {
    ...
    const text = [
      ...
      ...this.rows().map((row) =>
        columns.map((column) => this.cellValue(row, column.key)).join("\t"),
      ),
    ].join("\n");
  ...
  requestAnalysis(operation: "pivot" | "aggregate" | "formula"): void {
    this.analysisRequested.emit({
      operation,
      query: this.query(),
      columns: this.visibleColumns().map((column) => column.key),
    });
  ...
  totalRows(): number {
    return this.mode() === "server" && this.totalItems() !== null
      ? this.totalItems()!
      : this.rows().length;
  }
  mobileLoadedCount(): number {
    return this.rows().length;
  }
  mobileTotalLabel(): number | string {
    return this.totalItems() === null ? this.rows().length : this.totalItems()!;
  }
  sortStates(): HisHopeDataTableSort[] {
    const value = this.sort() ?? this.localSort();
    return !value ? [] : Array.isArray(value) ? value : [value];
  }
  sortState(): HisHopeDataTableSort | null {
    return this.sortStates()[0] ?? null;
  }
  pageValue(): number {
    return this.page() > 1 ? this.page() : this.localPage();
  }
  pageSizeValue(): number {
    return this.pageSize() !== 10 ? this.pageSize() : this.localPageSize();
  }
  pageCount(): number {
    return Math.max(
      1,
      Math.ceil(this.totalRows() / Math.max(1, this.pageSizeValue())),
    );
  }
  ...
  changeCursor(cursor: string | null): void {
    this.queryChange.emit(createHisHopeCursorQuery(this.query(), cursor));
  }
  pageStart(): number {
    return this.totalRows()
      ? (this.pageValue() - 1) * this.pageSizeValue() + 1
      : 0;
  }
  pageEnd(): number {
    return Math.min(this.totalRows(), this.pageValue() * this.pageSizeValue());
  }
  sortedRows(): Record<string, unknown>[] {
    const active = this.sortStates();
    if (this.mode() === "server" || !active.length) return this.rows();
    return [...this.rows()].sort((a, b) =>
      compareRowsBySortTerms(a, b, active),
    );
  }
  pagedRows(): Record<string, unknown>[] {
    const rows = this.flattenTree(this.sortedRows());
    if (this.mode() === "server" || this.virtualize()) return rows;
    return rows.slice(
      (this.pageValue() - 1) * this.pageSizeValue(),
      this.pageValue() * this.pageSizeValue(),
    );
  }
  private treeDepths = new Map<string, number>();
  treeDepth(row: Record<string, unknown>): number {
    return this.treeDepths.get(this.rowKeyValue(row)) ?? 0;
  }
  private flattenTree(
    ...
    const append = (items: Record<string, unknown>[], depth: number) =>
      items.forEach((item) => {
        this.treeDepths.set(this.rowKeyValue(item), depth);
        output.push(item);
  ...
  virtualWindowSize(): number {
    return Math.max(
      1,
      Math.ceil(560 / Math.max(24, this.virtualRowHeight())) + 4,
    );
  }
  renderedRows(): Record<string, unknown>[] {
    const rows = this.pagedRows();
    return this.virtualize()
      ? rows.slice(this.virtualStart(), this.virtualEnd())
      : rows;
  }
  onVirtualScroll(event: Event): void {
    ...
    if (this.virtualizeColumns()) {
      ...
      this.columnVirtualStart.set(
        Math.max(0, Math.floor(target.scrollLeft / 160) - 2),
      );
    }
    ...
    const next = Math.max(
      0,
      Math.min(
        Math.floor(target.scrollTop / Math.max(24, this.virtualRowHeight())),
        Math.max(0, this.pagedRows().length - this.virtualWindowSize()),
      ),
    );
    if (next !== this.virtualStart()) this.virtualStart.set(next);
  }
  rowKeyValue(row: Record<string, unknown>): string {
    return String(row["id"] ?? row["key"] ?? JSON.stringify(row));
  }
  ...
  detailTemplate(): HisHopeDataTableDetailDirective | null {
    return this.detailTemplates?.first ?? null;
  }
  isExpanded(row: Record<string, unknown>): boolean {
    return this.expandedRowKeys().includes(this.rowKeyValue(row));
  }
  toggleExpanded(row: Record<string, unknown>): void {
    const rowKey = this.rowKeyValue(row);
    this.rowExpandChange.emit({ rowKey, expanded: !this.isExpanded(row) });
  }
  ...
  toggleColumn(key: string): void {
    ...
    if (column && this.isResponsiveHidden(column)) {
      this.responsiveVisibleColumns.update((current) =>
  ...
  private isResponsiveHidden(column: HisHopeDataTableColumn): boolean {
    return isResponsiveColumnHidden(
      column.responsivePriority,
      this.viewportWidth(),
    );
  }
  beginColumnDrag(key: string): void {
  ...
  dropColumn(targetKey: string): void {
    ...
    const configuredOrder = this.localColumnOrder().length
      ? this.localColumnOrder()
      : this.columnOrder();
    ...
    this.localColumnOrder.set(order);
  ...
  beginColumnResize(event: PointerEvent, column: HisHopeDataTableColumn): void {
    event.preventDefault();
    event.stopPropagation();
    this.resizingColumnKey = column.key;
    this.resizeStartX = event.clientX;
    this.resizeStartWidth = this.columnWidth(column) ?? 160;
  }
  @HostListener("document:pointermove", ["$event"]) onColumnResize(
    event: PointerEvent,
  ): void {
    if (!this.resizingColumnKey) return;
    const column = this.columns().find(
      (item) => item.key === this.resizingColumnKey,
    );
    if (!column) return;
    ...
    this.localColumnWidths.update((widths) => ({
      ...widths,
      [column.key]: width,
    }));
  }
  @HostListener("document:pointerup") endColumnResize(): void {
    if (!this.resizingColumnKey) return;
    const key = this.resizingColumnKey;
    const width = this.localColumnWidths()[key];
    this.resizingColumnKey = null;
    if (width) this.columnResizeChange.emit({ key, width });
  }
  resizeByKeyboard(
    event: KeyboardEvent,
    column: HisHopeDataTableColumn,
    ...
    event.preventDefault();
    const width = this.clampColumnWidth(
      (this.columnWidth(column) ?? 160) + delta,
      column,
    );
    this.localColumnWidths.update((widths) => ({
      ...widths,
    ...
    this.columnResizeChange.emit({ key: column.key, width });
  }
  private clampColumnWidth(
    width: number,
    column: HisHopeDataTableColumn,
  ): number {
    return clampColumnWidthPure(width, column);
  }
  exportFormat(): HisHopeTableExportFormat {
    return this.exportFormats().includes(this.localExportFormat())
      ? this.localExportFormat()
      : (this.exportFormats()[0] ?? "csv");
  }
  changeExportFormat(event: Event): void {
    this.localExportFormat.set(
      (event.target as HTMLSelectElement).value as HisHopeTableExportFormat,
    );
  }
  requestExport(): void {
    const selected = this.selectedRowsValue();
    const rows = selected.length ? selected : this.rows();
    this.exportRequested.emit({
      format: this.exportFormat(),
      columns: this.visibleColumns()
        .filter((column) => column.exportable !== false)
        .map((column) => column.key),
      rowKeys: rows.map((row) => this.rowKeyValue(row)),
      query: this.query(),
      async: this.mode() === "server" || this.rows().length > 1000,
      maskSensitive: this.maskSensitiveExport(),
    });
  }
  completedRows(job: HisHopeBulkJob): number {
    return (
      job.rowProgress?.filter((item) => item.status === "completed").length ??
      job.processed
    );
  }
  sortBy(key: string, event?: Event): void {
    const next = toggleHisHopeDataTableSort(
      this.sortStates(),
      key,
    ...
    this.emitQuery(1, this.pageSizeValue(), next);
  }
  sortIcon(key: string): string {
    const active = this.sortStates().find((item) => item.key === key);
    if (!active) return "unfold_more";
    return active.direction === "asc" ? "arrow_upward" : "arrow_downward";
  }
  sortPriority(key: string): number | null {
    const index = this.sortStates().findIndex((item) => item.key === key);
    return index < 0 ? null : index + 1;
  }
  sortLabel(column: HisHopeDataTableColumn): string {
    const index = this.sortStates().findIndex(
      (item) => item.key === column.key,
    );
    if (index < 0)
      return `Sort by ${column.label} ascending. Hold Shift to add a sort level.`;
  ...
  changePage(page: number): void {
    this.localPage.set(Math.max(1, Math.min(page, this.pageCount())));
    this.pageChange.emit(this.localPage());
    this.emitQuery(this.localPage(), this.pageSizeValue());
  }
  changePageSize(event: Event): void {
    ...
    this.emitQuery(1, size);
  }
  onRowClick(row: Record<string, unknown>): void {
    if (this.rowClickable()) this.rowClick.emit(row);
  }
  onMobileRowClick(row: Record<string, unknown>): void {
  ...
  isEditing(row: Record<string, unknown>): boolean {
    return this.editingRowKey() === this.rowKeyValue(row);
  }
  isSaving(row: Record<string, unknown>): boolean {
    return (
      this.savingRowKey() === this.rowKeyValue(row) ||
      (this.editState()?.rowKey === this.rowKeyValue(row) &&
        this.editState()?.status === "saving")
    );
  }
  isDirty(row: Record<string, unknown>): boolean {
    return this.editState()?.rowKey === this.rowKeyValue(row)
      ? this.editState()?.dirty === true
      : JSON.stringify(this.draftRow()) !== JSON.stringify(row) &&
          this.isEditing(row);
  }
  editErrorMessage(): string {
    return this.editState()?.rowKey === this.editingRowKey()
      ? (this.editState()?.error ?? "")
      : "";
  }
  draftValue(key: string): string {
    return String(this.draftRow()[key] ?? "");
  }
  editError(key: string): string {
    return this.draftErrors()[key] ?? "";
  }
  hasEditErrors(): boolean {
    return Object.keys(this.draftErrors()).length > 0;
  }
  startEdit(row: Record<string, unknown>): void {
    this.editingRowKey.set(this.rowKeyValue(row));
    this.draftRow.set({ ...row });
    this.draftErrors.set({});
    this.previousEditRow = { ...row };
  }
  updateDraftValue(key: string, value: unknown): void {
    ...
    this.draftRow.set(next);
    this.draftErrors.set(errors);
  }
  updateDraft(key: string, event: Event): void {
  ...
  saveEdit(row: Record<string, unknown>): void {
    if (this.hasEditErrors() || this.savingRowKey() === this.rowKeyValue(row))
      return;
    const current = { ...row, ...this.draftRow() };
    const previous = this.previousEditRow ?? row;
    const request = {
      rowKey: this.rowKeyValue(row),
      previous,
      current,
      concurrencyToken:
        String(row["concurrencyToken"] ?? row["version"] ?? "") || undefined,
    };
    this.lastSaveRequest = request;
    this.lastEdit.set({ previous, current });
    this.rowEditSave.emit(current);
    this.rowEditSaveRequested.emit(request);
    if (!this.editState()) {
      this.editingRowKey.set(null);
      this.draftRow.set({});
      this.draftErrors.set({});
      this.previousEditRow = null;
    } else this.previousEditRow = null;
  }
  retryEditSave(): void {
    if (!this.lastSaveRequest) return;
    const current =
      this.editingRowKey() === this.lastSaveRequest.rowKey
        ? { ...this.lastSaveRequest.current, ...this.draftRow() }
        : this.lastSaveRequest.current;
  ...
  undoEdit(transaction: {
    ...
    this.rowEditUndo.emit(transaction);
    this.lastEdit.set(null);
  }
  resolveConflict(resolution: HisHopeConflictResolution): void {
    ...
    if (resolution === "latest") {
      ...
      const merged = fields.reduce(
        (result, key) => ({
          ...result,
          [key]: submitted[key] ?? conflict.latest[key],
        }),
        { ...conflict.latest },
      );
    ...
    if (resolution === "mine" || resolution === "merge") {
      ...
      this.lastSaveRequest = {
        ...
        previous: { ...conflict.latest },
        current,
        concurrencyToken: conflict.latestConcurrencyToken,
      };
      this.rowEditSaveRequested.emit(this.lastSaveRequest);
    }
  }
  ...
  isSelected(row: Record<string, unknown>): boolean {
    return this.selectedRows.has(this.rowKeyValue(row));
  }
  allVisibleSelected(): boolean {
    return (
      this.pagedRows().length > 0 &&
      (this.selectionScope() === "all" && this.selectAllPages
        ? this.pagedRows().every(
            (row) => !this.excludedRows.has(this.rowKeyValue(row)),
          )
        : this.pagedRows().every((row) => this.isSelected(row)))
    );
  }
  toggleRow(row: Record<string, unknown>): void {
    const key = this.rowKeyValue(row);
    if (this.selectionScope() === "all" && this.selectAllPages) {
      this.excludedRows.has(key)
        ? this.excludedRows.delete(key)
        : this.excludedRows.add(key);
    } else {
      this.isSelected(row)
    ...
    if (this.selectionScope() === "all") {
      ...
      this.allVisibleSelected()
        ? this.pagedRows().forEach((row) =>
            this.selectedRows.delete(this.rowKeyValue(row)),
          )
        : this.pagedRows().forEach((row) =>
            this.selectedRows.add(this.rowKeyValue(row)),
          );
    }
    this.emitSelection();
    this.emitSelectionState();
  }
  selectedRowsValue(): Record<string, unknown>[] {
    return this.rows().filter((row) =>
      this.selectionScope() === "all" && this.selectAllPages
        ? !this.excludedRows.has(this.rowKeyValue(row))
        : this.selectedRows.has(this.rowKeyValue(row)),
    );
  }
  hasSelection(): boolean {
    return this.selectAllPages || this.selectedRows.size > 0;
  }
  selectionCount(): string | number {
    return this.selectAllPages ? "all" : this.selectedRows.size;
  }
  runBulkAction(action: HisHopeBulkAction): void {
    const rows = this.selectedRowsValue();
    action.execute?.(rows);
    this.bulkActionRequested.emit({
      actionId: action.id,
      rowKeys: rows.map((row) => this.rowKeyValue(row)),
      query: this.query(),
      selection: this.selectionStateValue(),
    });
  }
  private emitQuery(
    page: number,
    pageSize: number,
    sort: HisHopeSort | null = this.sort() ?? this.localSort(),
  ): void {
    this.queryChange.emit({
      ...this.query(),
      page,
  ...
  private emitSelection(): void {
    this.selectionChange.emit(
      this.rows().filter((row) => this.selectedRows.has(this.rowKeyValue(row))),
    );
  }
  private selectionStateValue(): HisHopeTableSelectionState {
    return {
      scope: this.selectionScope(),
      selectedKeys: this.selectAllPages ? [] : [...this.selectedRows],
      excludedKeys: [...this.excludedRows],
      query: this.query(),
    };
  }
  private emitSelectionState(): void {
    this.selectionStateChange.emit(this.selectionStateValue());
  }
}
