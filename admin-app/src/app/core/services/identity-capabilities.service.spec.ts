...
describe("IdentityCapabilitiesService", () => {
  ...
  beforeEach(() => {
    const api = {
      getDevicePosturePolicy: () =>
        of({
          id: "default",
          mode: "observe",
          providers: ["advanced-compliance"],
      ...
      getIdentitySettings: () =>
        of([{ key: "PROVISIONING_MODE", value: "dry-run" }]),
      getAuditLogs: () =>
      ...
      getRadiusEapTlsStatus: () =>
        of({
          enabled: false,
          trustedCaConfigured: false,
          trustedCaReachable: false,
    ...
    } as unknown as IdentityCapabilitiesApiService;
    TestBed.configureTestingModule({
      providers: [
        IdentityCapabilitiesService,
        ApiErrorMessageService,
        { provide: IdentityCapabilitiesApiService, useValue: api },
      ],
    });
  ...
  it("normalizes unknown failures without exposing response bodies", () => {
    expect(service.normalizeError(new Error("secret-token"))).toEqual({
      status: 0,
      code: "network_error",
