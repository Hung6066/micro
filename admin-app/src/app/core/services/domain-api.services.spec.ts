...
describe("domain API HTTP contracts", () => {
  ...
  const cache = jasmine.createSpyObj<HisHopeRequestCacheService>("cache", [
    "getOrLoad",
    "invalidate",
  ]);
  const tableApi = jasmine.createSpyObj<AdminTableApiService>("tableApi", [
    "bulk",
    "export",
    "getViews",
    "saveView",
  ...
  beforeEach(() => {
    cache.getOrLoad.and.callFake(
      (_key: string, loader: () => unknown) => loader() as never,
    );
  ...
  it("sends the clients page query and normalizes client type", () => {
    TestBed.inject(ClientsApiService)
      .getClientsPage({ page: 2, pageSize: 25, search: " clinic " })
      .subscribe((result) => {
        expect(result.items[0].clientType).toBe("public");
      });
    const request = http.expectOne(
      (item) => item.url === `${environment.adminApiUrl}/clients`,
    );
