...
describe("capabilityPermissionGuard", () => {
  it("allows a capability route when the foundation snapshot has the read permission", (done) => {
    const permissions = {
      has: (permission: string) => permission === "admin.settings.read",
      setSnapshot: jasmine.createSpy(),
    };
    ...
    TestBed.configureTestingModule({
      providers: [
        { provide: HisHopePermissionService, useValue: permissions },
        {
          ...
          useValue: { getCurrent: () => of({ roles: [], permissions: [] }) },
        },
        { provide: Router, useValue: router },
      ],
    });

    TestBed.runInInjectionContext(() => capabilityPermissionGuard()).subscribe(
      (result) => {
