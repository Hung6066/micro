...
describe("RoleEditDialogComponent", () => {
  ...
  function configure(
    data: Role | null = null,
  ): jasmine.SpyObj<RolesApiService> {
    const api = jasmine.createSpyObj<RolesApiService>("RolesApiService", [
      "getPermissions",
      "getRoleOwners",
      "createRole",
      "updateRole",
    ...
    api.getRoleOwners.and.returnValue(
      of([{ key: "identity-service", name: "identity-service" }]),
    );
    api.createRole.and.returnValue(
      of({ id: "new-role", name: "Role" } as Role),
    );
  ...
  it("loads catalog, groups permissions and preserves selected role permissions", () => {
    ...
    expect(component.selectedPermissionCodes).toEqual(
      new Set(["clinical.read"]),
    );
    expect(
      component.isGroupPartiallySelected(component.permissionGroups[1]),
    ).toBeTrue();
  });
