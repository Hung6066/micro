...
import { HisHopeBulkAction, HisHopeBulkActionRequest, HisHopeDataTableCellDirective, HisHopeDataTableComponent, HisHopeDataTableColumn, HisHopeDataTableDetailDirective, HisHopeI18nService, HisHopePageHeaderComponent, HisHopePageLayoutComponent, HisHopePermissionService, HisHopeTableExportRequest, HisHopeToolbarComponent, HisHopeTranslatePipe } from '@his-hope/frontend-foundation';
...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, MatButtonModule, MatDialogModule, MatIconModule, MatSnackBarModule, HisHopeDataTableCellDirective, HisHopeDataTableComponent, HisHopeDataTableDetailDirective, HisHopePageHeaderComponent, HisHopePageLayoutComponent, HisHopeToolbarComponent, HisHopeTranslatePipe],
  template: `
    <hh-page-layout>
      ...
      <hh-toolbar hhPageToolbar [label]="'admin.users' | hhTranslate">
        ...
        <button *ngIf="canWrite" hh-toolbar-actions type="button" class="hh-button hh-button--primary" (click)="openCreateDialog()">
          <span class="material-icons" aria-hidden="true">person_add</span> {{ 'admin.createUser' | hhTranslate:'Create user' }}
        </button>
        <button hh-toolbar-actions type="button" class="hh-icon-button" (click)="loadUsers()" [attr.aria-label]="'admin.refresh' | hhTranslate" [attr.title]="'admin.refresh' | hhTranslate">
      ...
      <hh-data-table label="Users" [columns]="columns" [rows]="tableRows" [selection]="true" [loading]="loading"
        ...
        [exportable]="canExport" [bulkActions]="bulkActions" [filterBuilder]="true" [virtualizeColumns]="true" [expandedRowKeys]="expandedRowKeys" viewStorageKey="admin.users" viewName="default" [serverBackedView]="true" [savedView]="savedView"
        ...
        <ng-template hhDataTableCell="actions" let-row>
          <button *ngIf="canWrite" mat-icon-button type="button" (click)="openEditDialog(row)" [attr.aria-label]="'admin.edit' | hhTranslate">
            <mat-icon>edit</mat-icon>
          </button>
          <button *ngIf="canWrite" mat-icon-button type="button" (click)="toggleUser(row)" [attr.aria-label]="(row['isActive'] === 'Yes' ? 'admin.deactivate' : 'admin.activate') | hhTranslate">
            <mat-icon>{{ row['isActive'] === 'Yes' ? 'person_off' : 'person_add' }}</mat-icon>
          </button>
        </ng-template>
...
export class UsersPageComponent implements OnInit {
  ...
  get columns(): HisHopeDataTableColumn[] { this.i18n.locale(); return [
    ...
    { key: 'email', label: this.i18n.t('admin.email'), sortable: true, responsivePriority: 2 }, { key: 'roles', label: this.i18n.t('admin.roles'), responsivePriority: 3 }, { key: 'isActive', label: this.i18n.t('admin.active'), responsivePriority: 2, pinned: 'right' }, { key: 'actions', label: this.i18n.t('admin.actions', 'Actions'), hideable: false, sortable: false, reorderable: false, width: '96px', minWidth: 96, align: 'center' as const, responsivePriority: 1, pinned: 'right' },
  ]; }
  ...
  get bulkActions(): HisHopeBulkAction[] { this.i18n.locale(); return this.permissions.has('admin.users.write') ? [
  ...
  loadUsers(query = this.query): void {
    ...
    this.pageRequest = this.api.getUsersPage(query).pipe(finalize(() => this.loading = false), catchError(() => { this.error = 'Failed to load users.'; return of<AdminPageResult<User>>({ items: [], totalCount: 0, page: query.page, pageSize: query.pageSize, totalPages: 0, hasNextPage: false, hasPreviousPage: false }); }))
      .subscribe(result => {
        ...
        this.tableRows = result.items.map(user => ({ id: user.id, userName: user.userName, email: user.email, roles: (user.roles || []).join(', '), isActive: user.isActive ? 'Yes' : 'No', entity: user }));
      });
  }

  openCreateDialog(): void {
    if (!this.canWrite) return;
    this.dialog.open(UserEditDialogComponent, { width: 'min(720px, calc(100vw - 32px))', maxWidth: 'calc(100vw - 32px)', autoFocus: 'first-tabbable', restoreFocus: true, data: null })
      .afterClosed().subscribe(saved => { if (saved) this.loadUsers(this.query); });
  }
  ...
  openEditDialog(row: Record<string, unknown>): void {
    ...
    this.api.getUser(user.id).subscribe({
      next: detail => this.dialog.open(UserEditDialogComponent, { width: 'min(720px, calc(100vw - 32px))', maxWidth: 'calc(100vw - 32px)', autoFocus: 'first-tabbable', restoreFocus: true, data: detail }).afterClosed().subscribe(saved => { if (saved) this.loadUsers(this.query); }),
      error: () => this.snackBar.open(this.i18n.t('admin.loadUserFailed', 'Failed to load user.'), this.i18n.t('admin.close', 'Close'), { duration: 3000 }),
    });
  }
  ...
  toggleUser(row: Record<string, unknown>): void {
    ...
    request.subscribe({ next: () => this.loadUsers(this.query), error: () => this.snackBar.open(this.i18n.t('admin.updateUserFailed', 'Failed to update user.'), this.i18n.t('admin.close', 'Close'), { duration: 3000 }) });
  }
