  ...
  template: `
    ...
    @if (showBreakGlass) {
      <section class="grid two-col"><mat-card><mat-card-header><mat-card-title>{{ "admin.breakGlass" | hhTranslate: "Break-glass access" }}</mat-card-title></mat-card-header><mat-card-content class="form-grid">
        ...
        <mat-form-field appearance="outline"><mat-label>{{ "admin.permission" | hhTranslate: "Permission" }}</mat-label><mat-select [(ngModel)]="breakGlass.permissionCode"><mat-option *ngFor="let permission of permissions" [value]="permission.code">{{ permission.name }} · {{ permission.code }}</mat-option></mat-select></mat-form-field>
