...
@Component({
  ...
  template: `
    <hh-page-layout
      ><hh-page-header
        hhPageHeader
        [title]="'admin.accessRequests' | hhTranslate: 'Access requests'"
        [subtitle]="
      ...
      <p class="notice">
        {{
          "admin.governanceBoundary"
            | hhTranslate
              : "Approval, MFA and separation-of-duties checks remain server-side."
        }}
      </p>
...
export class AccessRequestsPageComponent implements OnInit {
  ...
  readonly state = new AdminResourceStateController<{
    ...
    loadErrorFallback: "Unable to load access request data.",
  });
  users: User[] = [];
  roles: Role[] = [];
  requests: AccessRequest[] = [];
