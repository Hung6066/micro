...
@Component({
  ...
  templateUrl: "./maintenance-work-page.component.html",
  styleUrls: ["./maintenance-work-page.component.scss"],
})
export class MaintenanceWorkPageComponent {
  activeTab: "work" | "machine" | "downtime" = "work";
  ...
  maintenanceTypeLabel(type: string): string {
    return manufacturingEnumLabel(this.i18n, "maintenanceType", type);
  }
  machineStateLabel(state: string): string {
    return manufacturingEnumLabel(this.i18n, "machineState", state);
  }
  calibrationResultLabel(result: string): string {
    return manufacturingEnumLabel(this.i18n, "qualityTestResult", result);
  }
  calibrationTypeLabel(type: string): string {
    return manufacturingEnumLabel(this.i18n, "calibrationTypes", type);
  }
  calibrationDueLabel(value: string): string {
    return this.i18n.formatDate(value, { dateStyle: "medium" });
  }
  telemetryObservedLabel(value: string): string {
  ...
  hasOpenDowntime(): boolean {
    return this.downtimes.some((item) => item.status === "Open");
  }
  ...
  updateChecklistItem(index: number, complete: boolean): void {
    ...
    this.checklistComplete = this.checklistItems.every(
  ...
  loadCalibrations(): void {
    ...
    this.api
      .getMachineCalibrations(machineId)
      .pipe(
        catchError((error) => {
      ...
      this.api
        ...
        .subscribe((machines) => {
          setTimeout(() => {
            ...
            if (
              this.machineId &&
              !machines.some((machine) => machine.id === this.machineId)
            )
              this.machineId = "";
      ...
      this.api
        ...
        .subscribe((orders) => {
          setTimeout(() => {
            ...
            if (
              this.workOrderId &&
              !orders.some((order) => order.id === this.workOrderId)
            )
              this.workOrderId = "";
  ...
  async completeWorkOrder(): Promise<void> {
    if (
      ...
      (this.checklistItems.length > 0 &&
        !this.checklistItems.every((item) => item.complete))
    ) {
      this.message = this.i18n.t(
        "mobile.operatorChecklistRequired",
        "Complete the checklist before closing the work order.",
      );
      return;
    }
    if (
      !this.machineId.trim() ||
      !this.workOrderId.trim() ||
      !this.technician.trim()
    ) {
      this.message = this.i18n.t(
        "mobile.operatorMaintenanceValidation",
        "Machine, work order and technician are required.",
      );
      return;
    }
    ...
    if (!scope) {
      this.message = this.i18n.t(
        ...
        "Sign in and select an operational tenant before completing maintenance.",
      );
      return;
    }
    const operation = await this.queue.submit(
      {
        ...scope,
        endpoint: `/machines/${this.machineId}/maintenance-work-orders/${this.workOrderId}/complete`,
        payload: {
          technician: this.technician.trim(),
          completedAt: new Date().toISOString(),
          evidence: this.evidence.trim() || undefined,
        },
      },
    ...
    this.message =
      operation.status === "synced"
        ? this.i18n.t(
            "mobile.operatorMaintenanceCompleted",
  ...
  async recordDowntime(): Promise<void> {
    ...
    if (!scope || !this.machineId.trim() || !this.downtimeReason.trim()) {
      this.message = this.i18n.t(
        "mobile.operatorDowntimeValidation",
        "Select a machine and enter a downtime reason.",
      );
      return;
    }
    const operation = await this.queue.submit(
      {
        ...scope,
        endpoint: `/machines/${this.machineId}/downtimes`,
        payload: {
    ...
    this.message =
      operation.status === "synced"
        ? this.i18n.t("mobile.operatorDowntimeRecorded", "Downtime recorded.")
        : this.i18n.t(
            "mobile.operatorPendingSync",
            "Pending sync — it will retry when connected.",
          );
  ...
  async resolveDowntime(): Promise<void> {
    ...
    if (!scope || !this.machineId.trim() || !downtime) {
      this.message = this.i18n.t(
        "mobile.operatorNoOpenDowntime",
        "No open downtime is available for this machine.",
      );
      return;
    }
    const operation = await this.queue.submit(
      {
        ...scope,
        endpoint: `/machines/${this.machineId}/downtimes/${downtime.id}/resolve`,
    ...
    this.message =
      operation.status === "synced"
        ? this.i18n.t("mobile.operatorDowntimeResolved", "Downtime resolved.")
        : this.i18n.t(
            "mobile.operatorPendingSync",
            "Pending sync — it will retry when connected.",
          );
