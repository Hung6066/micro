...
- ADR (placement tier): `docs/adr/018-tenant-placement-tier.md`
- Config: `config/conglomerate/customer-tenants.v1.json`
- Placement config (default off): `config/conglomerate/tenant-placement.v1.json`
...
## Tenant placement tier (ADR 018)

Default: **shared DB** for all tenants (`tenant-placement.v1.json` → `"enabled": false`).

Dedicated DB is **opt-in only** when contract/compliance/scale requires it:

1. Add tenant to `placements[]` with `tier: dedicated`, `active: true`, and per-service `connectionName`.
2. Provision `ConnectionStrings:{connectionName}` in the target service.
3. Set `"enabled": true` in `tenant-placement.v1.json` **and** `TenantPlacement:Enabled=true` in service config.
4. Run migrations against the dedicated database before cutover.

While `enabled=false`, dedicated entries are validated but **ignored at runtime** — all tenants stay on the default connection.

Full onboarding, provisioning, backup, and cutover steps: [tenant-placement-dedicated-tier.md](./tenant-placement-dedicated-tier.md).
