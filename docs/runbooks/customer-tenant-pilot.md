...
# ApiGateway (BFF session bridge for commerce — required for buyer + operator apps)

dotnet run --project src/ApiGateway



# Commerce API (behind gateway in production; direct for debugging only)
...
2. **Commerce audience** — CommerceService validates JWT audience **`commerce-api`** (still accepts `His.Hope` during transition).

3. **Portal PEP** — CommerceService rejects missing `portal_class`, blocks `end_user` from operator routes (`PATCH .../status`), and denies cross-tenant `?tenantKey=` for buyers.
