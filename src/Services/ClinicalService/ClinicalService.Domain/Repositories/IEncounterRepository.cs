...
public interface IEncounterRepository : IRepository<Encounter>
{
    ...
    Task<IReadOnlyList<Encounter>> GetByPatientIdAsync(
    ...
    Task<(IReadOnlyList<Encounter> Items, int TotalCount)> GetByPatientIdAsync(
        ...
        IReadOnlySet<string> facilityIds, bool crossFacility, CancellationToken ct = default);
    ...
    Task<(IReadOnlyList<Encounter> Items, int TotalCount)> SearchAsync(
