...
public static class TableViewEndpoints
{
    ...
    private static Guid? Subject(HttpContext context) => Guid.TryParse(context.User.FindFirstValue(ClaimTypes.NameIdentifier) ?? context.User.FindFirstValue("sub"), out var id) ? id : null;
    ...
    private static async Task<IResult> Save(string resource, string name, TableViewRequest request, IdentityDbContext db, HttpContext http, CancellationToken ct)
    {
        ...
        if (view is null) { view = new TableView { Id = Guid.NewGuid(), UserId = userId.Value, Resource = normalizedResource, Name = normalizedName }; db.TableViews.Add(view); }
    ...
    private static bool TryNormalize(string value, out string normalized)
    {
        ...
        return normalized.Length is > 0 and <= 80 && normalized.All(character => char.IsLetterOrDigit(character) || character is '-' or '_' or '.');
    }
