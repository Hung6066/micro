...
public static class IdentityWorkbenchDedicatedEndpoints
{
    ...
    {
        group.MapGet("/workload-sessions", async (
            IdentityDbContext db,
            IWorkloadSessionStore sessions,
            HttpContext http,
            CancellationToken ct) =>
        {
            var filter = IamTenantHttpContext.RequireFilter(http);

            var rolesQuery = db.IamWorkloadRoles.AsNoTracking().Where(role => role.IsActive);
            if (filter.AllowedScopeIds is not null)
                rolesQuery = rolesQuery.Where(role => filter.AllowedScopeIds.Contains(role.ScopeId));

            var roles = await rolesQuery.OrderBy(role => role.Key).ToListAsync(ct);
        ...
        group.MapGet("/revocations", async (IdentityDbContext db, HttpContext http, int? limit, CancellationToken ct) =>
        {
            var filter = IamTenantHttpContext.RequireFilter(http);
            ...
            var items = await db.AuditLogs.AsNoTracking()
                ...
                .WhereTenantActor(db, filter.AllowedTenantKeys)
        ...
        group.MapPost("/revocations", async (RevocationRequest request, IdentityDbContext db, ITokenBlacklistService blacklist, HttpContext http, CancellationToken ct) =>
        {
        ...
        group.MapGet("/analyzer/effective-access/{principalId:guid}", async (Guid principalId, IdentityDbContext db, HttpContext http, CancellationToken ct) =>
        {
            var filter = IamTenantHttpContext.RequireFilter(http);

            var isHuman = await db.Users.AsNoTracking().AnyAsync(x => x.Id == principalId && x.IsActive, ct);
            var workloadRole = isHuman
                ? null
                : await db.IamWorkloadRoles.AsNoTracking().SingleOrDefaultAsync(x => x.Id == principalId && x.IsActive, ct);
            if (!isHuman && workloadRole is null) return Results.NotFound("principal_not_found");
            if (isHuman)
            {
                if (await IamTenantAccessGuard.EnsureUserAccessAsync(db, principalId, filter, ct) is { } userError) return userError;
            }
            else if (IamTenantAccessGuard.EnsureScopeAccess(workloadRole!.ScopeId, filter) is { } scopeError) return scopeError;
        ...
        group.MapPost("/analyzer/policy-simulator", async (DedicatedPolicySimulationRequest request, IdentityDbContext db, HttpContext http, CancellationToken ct) =>
        {
            ...
            var filter = IamTenantHttpContext.RequireFilter(http);
            if (await IamTenantAccessGuard.EnsureUserAccessAsync(db, request.UserId, filter, ct) is { } userError) return userError;
        ...
        group.MapGet("/analyzer/unused-permissions", async (IdentityDbContext db, HttpContext http, CancellationToken ct) =>
        {
