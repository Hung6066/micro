...
{
    Task<LotDto> CreateLotAsync(CreateLotRequest request, CancellationToken cancellationToken = default);
    Task<(TransformationDto? Transformation, string? Error)> CreateTransformationAsync(CreateTransformationRequest request, CancellationToken cancellationToken = default);
    ...
    Task<(LotDto? Lot, string? Error)> SetLotDispositionAsync(Guid lotId, string disposition, string tenantKey, string? actor = null, string? reasonCode = null, string? evidenceReference = null, DateTimeOffset? expectedUpdatedAt = null, CancellationToken cancellationToken = default);
