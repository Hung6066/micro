...
internal static class ProductionEndpoints
{
    public static RouteGroupBuilder MapProductionEndpoints(this RouteGroupBuilder api)
    {
                ...
                api.MapGet("/transformations", (string? tenantKey, string? processStep, int? limit, int? page, HttpContext context, IManufacturingProductionStore store) =>
                {
                    ...
                    return Results.Ok(store.GetTransformationSummaries(processStep, limit ?? HisHopePaginationDefaults.DefaultPageSize, page ?? HisHopePaginationDefaults.FirstPage));
                });
                ...
                {
                    if (!TryResolveTenant(context, tenantKey, out var scopedTenant)) return Results.Forbid();
                    return Results.Ok(store.GetRecipes(productSku, active, limit ?? HisHopePaginationDefaults.DefaultPageSize, page ?? HisHopePaginationDefaults.FirstPage));
                });
                ...
                api.MapPost("/recipes", (CreateRecipeRequest request, HttpContext context, IManufacturingRecipeWorkflowStore store) =>
                {
                    if (string.IsNullOrWhiteSpace(request.TenantKey) || string.IsNullOrWhiteSpace(request.ProductSku) ||
                        ...
                        return ManufacturingProblem(StatusCodes.Status400BadRequest, ManufacturingErrorCodes.InvalidRecipe);
                    ...
                    catch (InvalidOperationException ex) when (ex.Message == ManufacturingErrorCodes.InvalidRecipeStatus)
                    { return ManufacturingProblem(StatusCodes.Status400BadRequest, ex.Message); }
                    catch (InvalidOperationException ex) when (ex.Message == ManufacturingErrorCodes.InvalidProductSpecification)
                    { return ManufacturingProblem(StatusCodes.Status400BadRequest, ex.Message); }
                });
                ...
                api.MapPost("/recipes/{recipeId:guid}/submit", (Guid recipeId, RecipeLifecycleRequest request, HttpContext context, IManufacturingRecipeWorkflowStore store) =>
                    ChangeRecipeLifecycle(recipeId, ManufacturingStatusCodes.Submitted, request, context, store));
                ...
                api.MapPost("/recipes/{recipeId:guid}/approve", (Guid recipeId, RecipeLifecycleRequest request, HttpContext context, IManufacturingRecipeWorkflowStore store) =>
                    ChangeRecipeLifecycle(recipeId, ManufacturingStatusCodes.Approved, request, context, store)).RequireAuthorization(AuthorizationPolicyNames.Permission(HisHopePermissions.Manufacturing.RecipeApprove));
                ...
                api.MapPost("/production-orders", (CreateProductionOrderRequest request, HttpContext context, IManufacturingProductionOrderStore store) =>
                {
                    ...
                    return result.Error switch
                    {
                        ...
                        ManufacturingErrorCodes.ProductionOrderExists => ManufacturingProblem(StatusCodes.Status409Conflict, result.Error!),
                        ManufacturingErrorCodes.RecipeNotFound => ManufacturingProblem(StatusCodes.Status404NotFound, result.Error!),
                        ManufacturingErrorCodes.RecipeUnavailable or ManufacturingErrorCodes.RecipeProductMismatch => ManufacturingProblem(StatusCodes.Status422UnprocessableEntity, result.Error!),
                ...
                api.MapPost("/production-batches", (CreateProductionBatchRequest request, HttpContext context, IManufacturingProductionOrderStore store) =>
                {
                    ...
                    return result.Error switch
                    {
                        ...
                        ManufacturingErrorCodes.TenantMismatch => Results.Forbid(),
                        ManufacturingErrorCodes.InvalidProductionBatch or "production_order_not_released" or ManufacturingErrorCodes.MachineUnavailable or ManufacturingErrorCodes.InputLotNotReleased or ManufacturingErrorCodes.InputReservationUnavailable or ManufacturingErrorCodes.InputReservationMismatch => ManufacturingProblem(StatusCodes.Status422UnprocessableEntity, result.Error!),
                        ManufacturingErrorCodes.ProductionBatchExists => ManufacturingProblem(StatusCodes.Status409Conflict, result.Error!),
                ...
                api.MapPost("/production-batches/{batchId:guid}/operations", (Guid batchId, RecordOperationRequest request, HttpContext context, IManufacturingProductionOrderStore store) =>
                {
                    ...
                    return result.Error switch
                    {
                        ManufacturingErrorCodes.ProductionBatchNotFound => ManufacturingProblem(StatusCodes.Status404NotFound, result.Error!),
                        ManufacturingErrorCodes.TenantMismatch => Results.Forbid(),
                        ManufacturingErrorCodes.BatchNotStarted or "invalid_operation_measurement" => ManufacturingProblem(StatusCodes.Status422UnprocessableEntity, result.Error!),
                        ManufacturingErrorCodes.OperationSequenceExists => ManufacturingProblem(StatusCodes.Status409Conflict, result.Error!),
                ...
                api.MapPost("/production-batches/{batchId:guid}/operations/{operationId:guid}/loss-review", (Guid batchId, Guid operationId, LossReviewRequest request, HttpContext context, IManufacturingRecipeWorkflowStore store) =>
                {
                    ...
                    return result.Error switch
                    {
                        ...
                        ManufacturingErrorCodes.ProductionBatchNotFound or ManufacturingErrorCodes.OperationNotFound => ManufacturingProblem(StatusCodes.Status404NotFound, result.Error!),
