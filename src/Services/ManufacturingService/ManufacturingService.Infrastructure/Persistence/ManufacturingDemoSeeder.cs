...
public static class ManufacturingDemoSeeder
{
    ...
    public static void Seed(IDbContextFactory<ManufacturingDbContext> dbFactory)
    {
        ...
        if (db.AuditEvents.Any(x => x.TenantKey == TenantKey && x.EntityType == "DemoSeed" && x.Action == "v1"))
        {
            ...
            var salesReservation = db.LotReservations.SingleOrDefault(x => x.Id == Id("reservation-sales-001"));
            ...
            if (!db.InspectionPlanVersions.Any(x => x.Id == Id("inspection-plan-mango-v1")))
                db.InspectionPlanVersions.Add(new ManufacturingInspectionPlanVersionEntity { Id = Id("inspection-plan-mango-v1"), TenantKey = TenantKey, PlanCode = "IP-MANGO", ProductSku = "FG-MANGO-CHILI", Version = 1, SamplingMethod = "Per lot", SamplingFrequency = "Every lot", AcceptanceCriteria = "Moisture <= 12%; sensory pass", Status = ManufacturingStatusCodes.Approved, EffectiveFrom = DateTimeOffset.UtcNow.AddDays(-30), ApprovedBy = "qa.demo", ApprovedAt = DateTimeOffset.UtcNow.AddDays(-30), CreatedBy = "seed", CreatedAt = DateTimeOffset.UtcNow.AddDays(-30) });
            ...
            if (finishedMango is not null && salesReservation is null)
                db.LotReservations.Add(new ManufacturingLotReservationEntity { Id = Id("reservation-sales-001"), TenantKey = TenantKey, LotId = finishedMango.Id, ReferenceType = "SalesOrder", ReferenceId = Id("sales-order-demo-001"), Quantity = 20, Uom = finishedMango.Uom, Status = "Reserved", CreatedAt = DateTimeOffset.UtcNow.AddHours(-2), ExpiresAt = DateTimeOffset.UtcNow.AddDays(7) });
            ...
            var supplier = db.Suppliers.SingleOrDefault(x => x.Id == Id("supplier-mekong"));
            if (supplier is not null && !db.SupplierMaterialApprovals.Any(x => x.SupplierId == supplier.Id))
                db.SupplierMaterialApprovals.AddRange(
                    new ManufacturingSupplierMaterialApprovalEntity { Id = Id("supplier-material-mango"), TenantKey = TenantKey, SupplierId = supplier.Id, MaterialSku = "RM-MANGO", ApprovedUom = "kg", EffectiveFrom = DateTimeOffset.UtcNow.AddDays(-30), Status = ManufacturingStatusCodes.Approved, Notes = "Demo approved raw mango", CreatedAt = DateTimeOffset.UtcNow, CreatedBy = "seed" },
                    new ManufacturingSupplierMaterialApprovalEntity { Id = Id("supplier-material-sugar"), TenantKey = TenantKey, SupplierId = supplier.Id, MaterialSku = "RM-SUGAR", ApprovedUom = "kg", EffectiveFrom = DateTimeOffset.UtcNow.AddDays(-30), Status = ManufacturingStatusCodes.Approved, Notes = "Demo approved sugar", CreatedAt = DateTimeOffset.UtcNow, CreatedBy = "seed" });
            if (supplier is not null && !db.SupplierCertificates.Any(x => x.Id == Id("supplier-certificate-mekong")))
                db.SupplierCertificates.Add(new ManufacturingSupplierCertificateEntity { Id = Id("supplier-certificate-mekong"), TenantKey = TenantKey, SupplierId = supplier.Id, CertificateType = "GlobalG.A.P.", CertificateNumber = "GAP-VN-2026-001", Issuer = "Vietnam Certification Authority", IssuedAt = DateTimeOffset.UtcNow.AddDays(-60), ExpiresAt = DateTimeOffset.UtcNow.AddDays(305), Status = "Active", EvidenceReference = "cert://demo/supplier-mekong-gap", CreatedAt = DateTimeOffset.UtcNow, CreatedBy = "seed" });
            if (demoBatch is not null && !db.OperationMeasurements.Any(x => x.Id == Id("operation-measurement-001")))
                db.OperationMeasurements.Add(new ManufacturingOperationMeasurementEntity { Id = Id("operation-measurement-001"), TenantKey = TenantKey, ProductionBatchId = demoBatch.Id, OperationExecutionId = Id("operation-001"), MachineId = machine?.Id, LotId = finishedMango?.Id, MeasurementType = "temperature_c", Value = 62.5m, Uom = "C", MeasuredAt = DateTimeOffset.UtcNow.AddHours(-2), RecordedBy = "operator.demo", Source = "telemetry", Sequence = 1, Notes = "Drying cycle midpoint", CreatedAt = DateTimeOffset.UtcNow.AddHours(-2) });
            if (!db.SalesActuals.Any(x => x.Id == Id("sales-actual-001")))
                db.SalesActuals.Add(new ManufacturingSalesActualEntity { Id = Id("sales-actual-001"), TenantKey = TenantKey, ProductSku = "FG-MANGO-CHILI", PeriodStart = DateOnly.FromDateTime(DateTime.UtcNow.AddDays(-30)), PeriodEnd = DateOnly.FromDateTime(DateTime.UtcNow.AddDays(-1)), Quantity = 240, Uom = "kg", Channel = "B2B", Region = "VN-South", Source = "sales.demo", Actor = "sales.demo", CreatedAt = DateTimeOffset.UtcNow });
            if (demoBatch is not null && !db.MlFeatureSnapshots.Any(x => x.Id == Id("ml-feature-snapshot-001")))
                db.MlFeatureSnapshots.Add(new ManufacturingMlFeatureSnapshotEntity { Id = Id("ml-feature-snapshot-001"), TenantKey = TenantKey, DatasetKey = "manufacturing-yield-v1", EntityType = "ProductionBatch", EntityId = demoBatch.Id, AsOf = DateTimeOffset.UtcNow, FeaturesJson = "{\"inputQuantity\":100,\"outputQuantity\":80,\"yieldPercent\":80,\"temperatureC\":62.5}", LabelJson = "{\"lossQuantity\":20}", SourceEventIdsJson = "[]", Split = "train", SchemaVersion = 1, CreatedBy = "seed", CreatedAt = DateTimeOffset.UtcNow });
            if (machine is not null && !db.MachineDowntimes.Any(x => x.Id == Id("machine-downtime-001")))
                db.MachineDowntimes.Add(new ManufacturingMachineDowntimeEntity { Id = Id("machine-downtime-001"), MachineId = machine.Id, TenantKey = TenantKey, Reason = "Planned sensor calibration", Status = ManufacturingStatusCodes.Closed, StartedAt = DateTimeOffset.UtcNow.AddDays(-3), EndedAt = DateTimeOffset.UtcNow.AddDays(-3).AddMinutes(45), Notes = "Scheduled preventive downtime" , CreatedAt = DateTimeOffset.UtcNow.AddDays(-3) });
            if (!db.MobileOperationReplays.Any(x => x.Id == Id("mobile-replay-001")))
                db.MobileOperationReplays.Add(new ManufacturingMobileOperationReplayEntity { Id = Id("mobile-replay-001"), TenantKey = TenantKey, SubjectId = "manufacturing.pilot", Method = "POST", Path = "/api/v1/manufacturing/production-batches/production-batch-001/operations", OperationId = "demo-replay-001", CreatedAt = DateTimeOffset.UtcNow.AddMinutes(-10) });
            db.SaveChanges();
            return;
        }
        ...
        db.ProductSpecifications.Add(new ManufacturingProductSpecificationEntity { Id = specId, TenantKey = TenantKey, ProductSku = "FG-MANGO-CHILI", TargetMoisturePercent = 12, Packaging = "500g stand-up pouch", ShelfLifeDays = 365, QcSpec = "Moisture <= 12%; sensory pass", Status = ManufacturingStatusCodes.Approved, ApprovedBy = "qa.demo", ApprovedAt = SeededAt, CreatedAt = SeededAt });
        db.InspectionPlanVersions.Add(new ManufacturingInspectionPlanVersionEntity { Id = Id("inspection-plan-mango-v1"), TenantKey = TenantKey, PlanCode = "IP-MANGO", ProductSku = "FG-MANGO-CHILI", Version = 1, SamplingMethod = "Per lot", SamplingFrequency = "Every lot", AcceptanceCriteria = "Moisture <= 12%; sensory pass", Status = ManufacturingStatusCodes.Approved, EffectiveFrom = SeededAt, ApprovedBy = "qa.demo", ApprovedAt = SeededAt, CreatedBy = "seed", CreatedAt = SeededAt });
        db.Recipes.Add(new ManufacturingRecipeEntity { Id = recipeId, TenantKey = TenantKey, ProductSku = "FG-MANGO-CHILI", Version = 1, ProcessStep = "drying", OutputUom = "kg", TargetYieldPercent = 80, Active = true, Status = ManufacturingStatusCodes.Approved, EffectiveFrom = SeededAt, ApprovedBy = "qa.demo", ApprovedAt = SeededAt, ProductSpecificationId = specId, CreatedAt = SeededAt, Components = [new ManufacturingRecipeComponentEntity { RecipeId = recipeId, IngredientSku = "RM-MANGO", Quantity = 5, Uom = "kg" }, new ManufacturingRecipeComponentEntity { RecipeId = recipeId, IngredientSku = "RM-SUGAR", Quantity = 0.2m, Uom = "kg" }] });
        ...
        db.Suppliers.Add(new ManufacturingSupplierEntity { Id = supplierId, TenantKey = TenantKey, Code = "SUP-MEKONG", Name = "Mekong Fresh Co.", LegalName = "Mekong Fresh Company Limited", TaxIdentificationNumber = "0312345678", ContactName = "Nguyen Van Anh", ContactEmail = "qa@mekongfresh.example", ContactPhone = "+84 28 5555 0101", CountryCode = "VN", Address = "Tien Giang, Vietnam", RiskLevel = "Standard", ApprovalStatus = ManufacturingStatusCodes.Approved, ApprovedBy = "qa.demo", ApprovedAt = SeededAt, LastReviewedAt = SeededAt, CreatedBy = "seed", UpdatedAt = SeededAt, Active = true, CreatedAt = SeededAt });
        db.SupplierMaterialApprovals.AddRange(
            new ManufacturingSupplierMaterialApprovalEntity { Id = Id("supplier-material-mango"), TenantKey = TenantKey, SupplierId = supplierId, MaterialSku = "RM-MANGO", ApprovedUom = "kg", EffectiveFrom = SeededAt.AddDays(-30), Status = ManufacturingStatusCodes.Approved, Notes = "Demo approved raw mango", CreatedAt = SeededAt, CreatedBy = "seed" },
            new ManufacturingSupplierMaterialApprovalEntity { Id = Id("supplier-material-sugar"), TenantKey = TenantKey, SupplierId = supplierId, MaterialSku = "RM-SUGAR", ApprovedUom = "kg", EffectiveFrom = SeededAt.AddDays(-30), Status = ManufacturingStatusCodes.Approved, Notes = "Demo approved sugar", CreatedAt = SeededAt, CreatedBy = "seed" });
        ...
        db.ProductionOrders.Add(new ManufacturingProductionOrderEntity { Id = productionOrderId, TenantKey = TenantKey, OrderNumber = "MO-2026-001", ProductSku = "FG-MANGO-CHILI", RecipeId = recipeId, RecipeVersion = 1, TargetQuantity = 80, OutputUom = "kg", Status = ManufacturingStatusCodes.Released, CreatedAt = SeededAt, ReleasedAt = SeededAt.AddHours(1) });
        db.ProductionBatches.Add(new ManufacturingProductionBatchEntity { Id = batchId, TenantKey = TenantKey, ProductionOrderId = productionOrderId, BatchNumber = "BATCH-2026-001", Status = ManufacturingStatusCodes.Started, PlannedQuantity = 80, ActualOutputQuantity = 80, MachineId = machineId, OutputLotId = outputLotId, CreatedAt = SeededAt.AddHours(1), StartedAt = SeededAt.AddHours(2) });
        ...
        db.LotReservations.Add(new ManufacturingLotReservationEntity { Id = Id("reservation-sales-001"), TenantKey = TenantKey, LotId = outputLotId, ReferenceType = "SalesOrder", ReferenceId = Id("sales-order-demo-001"), Quantity = 20, Uom = "kg", Status = "Reserved", CreatedAt = SeededAt.AddHours(5), ExpiresAt = SeededAt.AddDays(7) });
        ...
        db.LossReviews.Add(new ManufacturingLossReviewEntity { Id = Id("loss-review-001"), TenantKey = TenantKey, ProductionBatchId = batchId, OperationExecutionId = operationId, Decision = ManufacturingStatusCodes.Approved, Reviewer = "qa.demo", Notes = "Expected drying loss", ReviewedAt = SeededAt.AddHours(7) });
        ...
        db.QualityInspections.Add(new ManufacturingQualityInspectionEntity { Id = Id("inspection-001"), LotId = outputLotId, TenantKey = TenantKey, Status = ManufacturingStatusCodes.Approved, MoisturePercent = 11.4m, Inspector = "qa.demo", Notes = "Meets product specification", InspectedAt = SeededAt.AddHours(8) });
        ...
        db.InventoryTransactions.AddRange(
            new ManufacturingInventoryTransactionEntity { Id = Id("inventory-receipt-001"), TenantKey = TenantKey, LotId = rawLotId, TransactionType = "Receipt", Quantity = 100, Uom = "kg", FacilityId = "FAC-HCM", StockStatus = ManufacturingStatusCodes.Released, CorrelationId = Id("receipt-001"), OccurredAt = SeededAt.AddHours(1) },
            new ManufacturingInventoryTransactionEntity { Id = Id("inventory-reserve-001"), TenantKey = TenantKey, LotId = rawLotId, TransactionType = "Reserve", Quantity = 100, Uom = "kg", FacilityId = "FAC-HCM", StockStatus = ManufacturingStatusCodes.Released, CorrelationId = reservationId, OccurredAt = SeededAt.AddHours(1) },
            new ManufacturingInventoryTransactionEntity { Id = Id("inventory-output-001"), TenantKey = TenantKey, LotId = outputLotId, TransactionType = "ProductionOutput", Quantity = 80, Uom = "kg", FacilityId = "FAC-HCM", StockStatus = ManufacturingStatusCodes.Released, CorrelationId = batchId, OccurredAt = SeededAt.AddHours(6) });
    ...
    private static Guid Id(string value) => Guid.Parse(value switch
    {
        ...
        "production-order-001" => "10000000-0000-0000-0000-000000000070", "production-batch-001" => "10000000-0000-0000-0000-000000000071", "operation-001" => "10000000-0000-0000-0000-000000000072", "reservation-production-001" => "10000000-0000-0000-0000-000000000073", "reservation-sales-001" => "10000000-0000-0000-0000-000000000078", "sales-order-demo-001" => "10000000-0000-0000-0000-000000000079", "batch-input-001" => "10000000-0000-0000-0000-000000000074", "transformation-001" => "10000000-0000-0000-0000-000000000075", "loss-review-001" => "10000000-0000-0000-0000-000000000076", "production-batch-cost-001" => "10000000-0000-0000-0000-000000000077",
        "inspection-001" => "10000000-0000-0000-0000-000000000080", "deviation-001" => "10000000-0000-0000-0000-000000000081", "capa-001" => "10000000-0000-0000-0000-000000000082", "forecast-001" => "10000000-0000-0000-0000-000000000083", "supplier-certificate-mekong" => "10000000-0000-0000-0000-000000000084", "operation-measurement-001" => "10000000-0000-0000-0000-000000000085", "sales-actual-001" => "10000000-0000-0000-0000-000000000086", "ml-feature-snapshot-001" => "10000000-0000-0000-0000-000000000087", "machine-downtime-001" => "10000000-0000-0000-0000-000000000088", "mobile-replay-001" => "10000000-0000-0000-0000-000000000089",
...
#pragma warning disable EF1002 // table names come exclusively from the constant allow-list above.
            db.Database.ExecuteSqlRaw($"UPDATE {table} SET tenant_key = @p0 WHERE tenant_key = @p1", newTenant, oldTenant);
