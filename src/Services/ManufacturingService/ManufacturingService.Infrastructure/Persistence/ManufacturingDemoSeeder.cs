        ...
        if (db.AuditEvents.Any(x => x.TenantKey == TenantKey && x.EntityType == "DemoSeed" && x.Action == "v1"))
        {
            ...
            if (supplier is not null && !db.SupplierMaterialApprovals.Any(x => x.SupplierId == supplier.Id))
                db.SupplierMaterialApprovals.AddRange(
                    new ManufacturingSupplierMaterialApprovalEntity { Id = Id("supplier-material-mango"), TenantKey = TenantKey, SupplierId = supplier.Id, MaterialSku = "RM-MANGO", ApprovedUom = "kg", EffectiveFrom = DateTimeOffset.UtcNow.AddDays(-30), Status = "Approved", Notes = "Demo approved raw mango", CreatedAt = DateTimeOffset.UtcNow, CreatedBy = "seed" },
                    new ManufacturingSupplierMaterialApprovalEntity { Id = Id("supplier-material-sugar"), TenantKey = TenantKey, SupplierId = supplier.Id, MaterialSku = "RM-SUGAR", ApprovedUom = "kg", EffectiveFrom = DateTimeOffset.UtcNow.AddDays(-30), Status = "Approved", Notes = "Demo approved sugar", CreatedAt = DateTimeOffset.UtcNow, CreatedBy = "seed" });
