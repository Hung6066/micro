...
export class AdminService {
  ...
  getUsers(params?: {
    ...
    return this.http
      ...
      >(`${this.baseUrl}/users`, { params: httpParams })
      .pipe(retry(1), catchError(this.handleError));
  }

  getUser(id: string): Observable<AdminUser> {
    return this.http
      .get<AdminUser>(`${this.baseUrl}/users/${id}`)
      .pipe(retry(1), catchError(this.handleError));
  }
  ...
  createUser(
    ...
    return this.http
      .post<AdminUser>(`${this.baseUrl}/users`, data)
      .pipe(catchError(this.handleError));
  }

  updateUser(id: string, data: Partial<AdminUser>): Observable<AdminUser> {
    return this.http
      .put<AdminUser>(`${this.baseUrl}/users/${id}`, data)
      .pipe(catchError(this.handleError));
  }

  deactivateUser(id: string): Observable<void> {
    return this.http
      .post<void>(`${this.baseUrl}/users/${id}/deactivate`, {})
      .pipe(catchError(this.handleError));
  }

  activateUser(id: string): Observable<void> {
    return this.http
      .post<void>(`${this.baseUrl}/users/${id}/activate`, {})
      .pipe(catchError(this.handleError));
  }

  assignRoles(userId: string, roleIds: string[]): Observable<AdminUser> {
    return this.http
      .post<AdminUser>(`${this.baseUrl}/users/${userId}/roles`, { roleIds })
      .pipe(catchError(this.handleError));
  }
  ...
  getRole(id: string): Observable<Role> {
    return this.http
      .get<Role>(`${this.baseUrl}/roles/${id}`)
      .pipe(retry(1), catchError(this.handleError));
  }
  ...
  createRole(data: {
    ...
    return this.http
      .post<Role>(`${this.baseUrl}/roles`, data)
      .pipe(catchError(this.handleError));
  }
  ...
  updateRole(
    ...
    return this.http
      .put<Role>(`${this.baseUrl}/roles/${id}`, data)
      .pipe(catchError(this.handleError));
  }

  deleteRole(id: string): Observable<void> {
    return this.http
      .delete<void>(`${this.baseUrl}/roles/${id}`)
      .pipe(catchError(this.handleError));
  }

  getPermissions(): Observable<PermissionGroup[]> {
    return this.http
      .get<PermissionGroup[]>(`${this.baseUrl}/permissions`)
      .pipe(retry(1), catchError(this.handleError));
  }
  ...
  updateSetting(key: string, value: any): Observable<Setting> {
    return this.http
      .put<any>(`${this.baseUrl}/settings/${key}`, { value: String(value) })
      .pipe(
        map((s) => ({
          key: s.key,
          value: this.parseValue(s.value),
  ...
  bulkUpdateSettings(data: { key: string; value: any }[]): Observable<void> {
    ...
    return this.http
      .put<void>(`${this.baseUrl}/settings/bulk`, { settings: body })
      .pipe(catchError(this.handleError));
  }
  ...
  getAuditLogs(params?: {
    ...
    return this.http
      ...
      >(`${this.baseUrl}/audit-logs`, { params: httpParams })
      .pipe(retry(1), catchError(this.handleError));
  }

  getAuditLog(id: string): Observable<AuditLog> {
    return this.http
      .get<AuditLog>(`${this.baseUrl}/audit-logs/${id}`)
      .pipe(retry(1), catchError(this.handleError));
  }
  ...
  getDashboardStats(): Observable<AdminDashboardStats> {
    return this.http
      .get<AdminDashboardStats>(`${this.baseUrl}/dashboard`)
      .pipe(retry(1), catchError(this.handleError));
  }
  ...
  private handleError(error: HttpErrorResponse): Observable<never> {
    let errorMessage = this.i18n.t(
      "adminPage.errorUnknown",
      "Lỗi không xác định",
    );
    if (error.error instanceof ErrorEvent) {
      errorMessage = this.i18n.t(
        "adminPage.errorClient",
        "Lỗi máy khách: {{message}}",
        { message: error.error.message },
      );
    } else {
      errorMessage = this.i18n.t(
        "adminPage.errorServer",
        "Lỗi máy chủ: {{status}} - {{message}}",
