...
describe('permissionGuard', () => {
  ...
  beforeEach(() => {
    ...
    TestBed.configureTestingModule({
      providers: [
        { provide: HisHopePermissionService, useClass: HisHopePermissionService },
  ...
  it('allows activation when the shared snapshot has the permission', (done) => {
    ...
    TestBed.runInInjectionContext(() => permissionGuard(route, {} as any)).subscribe((result) => {
  ...
  it('redirects when the shared snapshot lacks the permission', (done) => {
    ...
    TestBed.runInInjectionContext(() => permissionGuard(route, {} as any)).subscribe(() => {
  ...
  it('redirects unauthenticated users to login', (done) => {
    ...
    router.createUrlTree.and.returnValue('/auth/login?returnUrl=%2Fpatients' as any);
    const route = { data: { permissions: ['patients.view'] } } as any as ActivatedRouteSnapshot;

    TestBed.runInInjectionContext(() => permissionGuard(route, { url: '/patients' } as any)).subscribe(() => {
  ...
  it('allows activation when no permission is required', (done) => {
    ...
    TestBed.runInInjectionContext(() => permissionGuard(route, { url: '/patients' } as any)).subscribe((result) => {
      expect(result).toBeTrue();
      done();
    });
  });
