...
export class UserFormDialogComponent implements OnDestroy {
  ...
  constructor(
    ...
    this.form = this.fb.group({
      ...
      phone: [
        user?.phone || "",
        [Validators.required, Validators.pattern(/^0[0-9]{9,10}$/)],
      ],
      password: [
        "",
        data.mode === "create"
  ...
  save(): void {
    ...
    if (this.data.mode === "create") {
      this.adminService
        ...
        .subscribe({
          ...
          error: () => {
            this.saving = false;
            this.snackBar.open(
              this.i18n.t("userForm.createFailed", "Không thể thêm người dùng"),
              this.i18n.t("common.close", "Đóng"),
              { duration: 5000 },
            );
            this.cdr.markForCheck();
      ...
      this.adminService
        .updateUser(this.data.user!.id, updateData)
        .pipe(takeUntil(this.destroy$))
        .subscribe({
          next: () => {
            this.snackBar.open(
              this.i18n.t(
                "userForm.updateSuccess",
