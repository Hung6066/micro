  ...
  template: `
    <hh-page-layout>
      <hh-page-header
        ...
        [subtitle]="
          'appointments.subtitle' | hhTranslate: 'Quản lý lịch hẹn khám bệnh'
        "
      >
        <button
          mat-raised-button
        ...
        >
          ...
          {{ "appointments.new" | hhTranslate: "Tạo lịch hẹn" }}
        </button>
      </hh-page-header>

      <hh-toolbar
        hhPageToolbar
        [label]="'appointments.toolbar' | hhTranslate: 'Danh sách lịch hẹn'"
      >
        <span hhToolbarTitle
          >{{ totalItems }}
          {{ "appointments.count" | hhTranslate: "cuộc hẹn" }}</span
        >
      </hh-toolbar>

      <hh-data-table
        [label]="'appointments.tableLabel' | hhTranslate: 'Danh sách lịch hẹn'"
        [loading]="loading"
        [error]="error"
        [empty]="rows.length === 0 && !loading"
        [emptyMessage]="
          'appointments.empty' | hhTranslate: 'Không tìm thấy lịch hẹn nào.'
        "
        [searchPlaceholder]="
          'appointments.search' | hhTranslate: 'Tìm theo mã bệnh nhân...'
        "
        [columns]="columns"
        [rows]="rows"
        [pageSize]="20"
        ...
        (retry)="loadData()"
      >
        <ng-template hhDataTableCell="scheduledDate" let-row>{{
          row["scheduledDate"] | date: "mediumDate"
        }}</ng-template>
        <ng-template hhDataTableCell="startTime" let-row
          >{{ row["startTime"] }} – {{ row["endTime"] }}</ng-template
        >
        <ng-template hhDataTableCell="type" let-row>{{
        ...
        <ng-template hhDataTableCell="reason" let-row>{{
          row["reason"] || "-"
        }}</ng-template>
        <ng-template hhDataTableCell="actions" let-row>
          <a
            mat-icon-button
            [routerLink]="['/appointments', idOf(row)]"
...
export class AppointmentListComponent implements OnInit, OnDestroy {
  ...
  columns: HisHopeDataTableColumn[] = [
    {
      key: "scheduledDate",
      label: this.i18n.t("appointments.column.date", "Ngày hẹn"),
      sortable: true,
    },
    ...
    {
      key: "status",
      label: this.i18n.t("appointments.column.status", "Trạng thái"),
      status: true,
    },
    {
      key: "reason",
      label: this.i18n.t("appointments.column.reason", "Lý do"),
    },
    {
      key: "actions",
