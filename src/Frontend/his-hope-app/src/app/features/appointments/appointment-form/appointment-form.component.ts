      ...
      <hh-page-header
        ...
        [subtitle]="
          'appointmentForm.subtitle'
            | hhTranslate: 'Lên lịch hẹn khám cho bệnh nhân'
        "
      />
          ...
          >
            <div class="form-grid">
              ...
              <mat-form-field appearance="outline" class="full-width">
                <mat-label>{{
                  "appointmentForm.patient" | hhTranslate: "Tìm bệnh nhân"
                }}</mat-label>
                <input
                  matInput
                  ...
                  required
                  [placeholder]="
                    'appointmentForm.patientPlaceholder'
                      | hhTranslate: 'Nhập tên bệnh nhân...'
                  "
                />
                <mat-icon matSuffix>search</mat-icon>
                <mat-hint>{{
                  "appointmentForm.patientHint"
                    | hhTranslate: "Gõ tên bệnh nhân để tìm kiếm"
                }}</mat-hint>
                <mat-autocomplete
                  #patientAuto="matAutocomplete"
                  [displayWith]="displayPatientName"
                >
                  @for (p of filteredPatients; track p.id) {
                    <mat-option [value]="p">
                      <span>{{ p.fullName }}</span>
                      <small class="option-detail">
                        - {{ p.genderName }}, {{ p.age }}t -
                        {{ p.phone }}</small
                      >
                    </mat-option>
                  }
                  @if (filteredPatients.length === 0 && patientSearchTerm) {
                    <mat-option disabled>
                      {{
                        "appointmentForm.noPatients"
                          | hhTranslate: "Không tìm thấy bệnh nhân"
                      }}
                    </mat-option>
                  }
                </mat-autocomplete>
              ...
              <mat-form-field appearance="outline">
                <mat-label>{{
                  "appointmentForm.provider" | hhTranslate: "Bác sĩ"
                }}</mat-label>
                <mat-select formControlName="providerId" required>
                  <mat-option [value]="" disabled>{{
                    "appointmentForm.providerPlaceholder"
                      | hhTranslate: "Chọn bác sĩ"
                  }}</mat-option>
                  @for (prov of providers; track prov.id) {
                    <mat-option [value]="prov.id">
                      {{ prov.fullName }}
                      @if (prov.specialty) {
                        <small>- {{ prov.specialty }}</small>
                      }
                    </mat-option>
                  }
                </mat-select>
              ...
              <mat-form-field appearance="outline">
                <mat-label>{{
                  "appointmentForm.date" | hhTranslate: "Ngày"
                }}</mat-label>
                <input
                  matInput
              ...
              <mat-form-field appearance="outline">
                <mat-label>{{
                  "appointmentForm.startTime" | hhTranslate: "Giờ bắt đầu"
                }}</mat-label>
                <input
                  matInput
                  type="time"
              ...
              <mat-form-field appearance="outline">
                <mat-label>{{
                  "appointmentForm.duration" | hhTranslate: "Thời lượng (phút)"
                }}</mat-label>
                <input
                  matInput
                  type="number"
              ...
              <mat-form-field appearance="outline">
                <mat-label>{{
                  "appointmentForm.type" | hhTranslate: "Loại"
                }}</mat-label>
                <mat-select formControlName="typeCode" required>
                  <mat-option value="CHECKUP">{{
                    "appointmentForm.type.checkup"
                      | hhTranslate: "Khám tổng quát"
                  }}</mat-option>
                  <mat-option value="CONSULT">{{
                    "appointmentForm.type.consult" | hhTranslate: "Tư vấn"
                  }}</mat-option>
                  <mat-option value="FOLLOWUP">{{
                    "appointmentForm.type.followup" | hhTranslate: "Tái khám"
                  }}</mat-option>
                  <mat-option value="EMERG">{{
                    "appointmentForm.type.emergency" | hhTranslate: "Cấp cứu"
                  }}</mat-option>
                  <mat-option value="PROCED">{{
                    "appointmentForm.type.procedure" | hhTranslate: "Thủ thuật"
                  }}</mat-option>
                  <mat-option value="VACCINE">{{
                    "appointmentForm.type.vaccine" | hhTranslate: "Tiêm chủng"
                  }}</mat-option>
                  <mat-option value="LAB">{{
                    "appointmentForm.type.lab" | hhTranslate: "Xét nghiệm"
                  }}</mat-option>
                  <mat-option value="TELE">{{
                    "appointmentForm.type.tele" | hhTranslate: "Khám từ xa"
                  }}</mat-option>
                </mat-select>
              </mat-form-field>
              ...
              <mat-form-field appearance="outline" class="full-width">
                <mat-label>{{
                  "appointmentForm.reason" | hhTranslate: "Lý do"
                }}</mat-label>
              ...
              <mat-form-field appearance="outline">
                <mat-label>{{
                  "appointmentForm.location" | hhTranslate: "Địa điểm"
                }}</mat-label>
          ...
          >
            @if (submitting) {
              <mat-spinner
                ...
                class="btn-spinner"
                [attr.aria-label]="
                  'common.saving' | hhTranslate: 'Đang đặt lịch'
                "
              ></mat-spinner>
            }
            {{
              submitting
                ? ("common.saving" | hhTranslate: "Đang đặt...")
                : ("appointmentForm.save" | hhTranslate: "Đặt lịch")
            }}
          </button>
        </div>
      </form>
    ...
    this.appointmentService
      ...
      .subscribe({
        ...
        error: () => {
          ...
          this.snackBar.open(
            this.i18n.t("appointmentForm.saveFailed", "Không thể đặt lịch hẹn"),
            this.i18n.t("common.close", "Đóng"),
            { duration: 5000 },
          );
          this.cdr.markForCheck();
