      ...
      <hh-page-header
        ...
        [subtitle]="
          'medications.subtitle'
            | hhTranslate: 'Quản lý danh mục thuốc của bệnh viện'
        "
      >
        <button
          mat-raised-button
          color="primary"
          routerLink="/pharmacy/medications/new"
          [attr.aria-label]="'medications.new' | hhTranslate: 'Thêm thuốc mới'"
        >
          <mat-icon>add</mat-icon>
          {{ "medications.new" | hhTranslate: "Thêm thuốc" }}
        </button>
      </hh-page-header>

      <hh-toolbar
        hhPageToolbar
        [label]="'medications.toolbar' | hhTranslate: 'Danh mục thuốc'"
      >
        <span hhToolbarTitle
          >{{ totalItems }}
          {{ "medications.count" | hhTranslate: "loại thuốc" }}</span
        >
      </hh-toolbar>

      <hh-data-table
        [label]="'medications.tableLabel' | hhTranslate: 'Danh mục thuốc'"
        [loading]="loading"
        [error]="error"
        [empty]="rows.length === 0 && !loading"
        [emptyMessage]="
          'medications.empty' | hhTranslate: 'Không tìm thấy thuốc nào.'
        "
        [searchPlaceholder]="
          'medications.search' | hhTranslate: 'Tên thuốc, hoạt chất, hoặc mã...'
        "
        [columns]="columns"
        [rows]="rows"
        [pageSize]="20"
        ...
        (retry)="loadData()"
      >
        <ng-template hhDataTableCell="name" let-row>{{
          row["name"]
        }}</ng-template>
        <ng-template hhDataTableCell="genericName" let-row>{{
          row["genericName"]
        }}</ng-template>
        <ng-template hhDataTableCell="strength" let-row>{{
          row["strength"]
        }}</ng-template>
        <ng-template hhDataTableCell="dosageForm" let-row>{{
          row["dosageForm"]
        }}</ng-template>
        <ng-template hhDataTableCell="isActive" let-row>
          <hh-status-badge
            [status]="isActiveOf(row) ? 'active' : 'inactive'"
            [label]="isActiveLabel(row)"
          />
        </ng-template>
        <ng-template hhDataTableCell="actions" let-row>
          <a
            mat-icon-button
            [routerLink]="['/pharmacy/medications', idOf(row)]"
          ...
          <a
            mat-icon-button
            color="accent"
            [routerLink]="['/pharmacy/medications', idOf(row), 'edit']"
            [attr.aria-label]="'common.edit' | hhTranslate: 'Chỉnh sửa'"
            (click)="$event.stopPropagation()"
          >
...
export class MedicationListComponent implements OnInit, OnDestroy {
  ...
  columns: HisHopeDataTableColumn[] = [
    {
      key: "name",
      label: this.i18n.t("medications.column.name", "Tên thuốc"),
      sortable: true,
    },
    {
      key: "genericName",
      label: this.i18n.t("medications.column.genericName", "Hoạt chất"),
    },
    {
      key: "strength",
      label: this.i18n.t("medications.column.strength", "Hàm lượng"),
    },
    {
      key: "dosageForm",
      label: this.i18n.t("medications.column.dosageForm", "Dạng bào chế"),
    },
    {
      key: "isActive",
      label: this.i18n.t("medications.column.status", "Trạng thái"),
      status: true,
    },
    {
      key: "actions",
