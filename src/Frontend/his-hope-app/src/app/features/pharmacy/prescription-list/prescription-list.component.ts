...
@Component({
    ...
    <hh-page-layout>
      <hh-page-header hhPageHeader
                      ...
                      [subtitle]="'prescriptions.subtitle' | hhTranslate:'Quản lý đơn thuốc theo yêu cầu'">
        <button mat-raised-button color="primary" routerLink="/pharmacy/prescriptions/new"
                [attr.aria-label]="'prescriptions.new' | hhTranslate:'Tạo đơn thuốc mới'">
          <mat-icon>add</mat-icon> {{ 'prescriptions.new' | hhTranslate:'Tạo đơn thuốc' }}
        </button>
      </hh-page-header>

      <hh-toolbar hhPageToolbar [label]="'prescriptions.toolbar' | hhTranslate:'Danh sách đơn thuốc'">
        <span hhToolbarTitle>{{ totalItems }} {{ 'prescriptions.count' | hhTranslate:'đơn thuốc' }}</span>
        <label class="hh-status-filter" hh-toolbar-controls>
          ...
          <select [value]="statusCode" (change)="onStatusFilterChange($event)"
                  [attr.aria-label]="'prescriptions.statusFilter' | hhTranslate:'Lọc theo trạng thái'">
            <option value="">{{ 'common.all' | hhTranslate:'Tất cả' }}</option>
            <option value="active">{{ 'prescriptions.status.active' | hhTranslate:'Đang hoạt động' }}</option>
            <option value="filled">{{ 'prescriptions.status.filled' | hhTranslate:'Đã cấp phát' }}</option>
            <option value="partially_filled">{{ 'prescriptions.status.partial' | hhTranslate:'Cấp phát một phần' }}</option>
            <option value="cancelled">{{ 'prescriptions.status.cancelled' | hhTranslate:'Đã hủy' }}</option>
            <option value="expired">{{ 'prescriptions.status.expired' | hhTranslate:'Hết hạn' }}</option>
          </select>
        </label>
      </hh-toolbar>

      <hh-data-table [label]="'prescriptions.tableLabel' | hhTranslate:'Danh sách đơn thuốc'"
                     [loading]="loading" [error]="error"
                     [empty]="rows.length === 0 && !loading"
                     [emptyMessage]="'prescriptions.empty' | hhTranslate:'Không tìm thấy đơn thuốc nào.'"
                     [searchPlaceholder]="'prescriptions.search' | hhTranslate:'Mã đơn, tên thuốc, bệnh nhân...'"
        ...
        <ng-template hhDataTableCell="medicationName" let-row>{{ row['medicationName'] }}</ng-template>
        <ng-template hhDataTableCell="strength" let-row>{{ row['strength'] }}</ng-template>
        <ng-template hhDataTableCell="quantity" let-row>{{ row['quantity'] }}</ng-template>
        <ng-template hhDataTableCell="prescribedAt" let-row>{{ row['prescribedAt'] | date:'mediumDate' }}</ng-template>
        <ng-template hhDataTableCell="statusCode" let-row>
          <hh-status-badge [status]="statusOf(row)" [label]="statusLabel(row)" />
        </ng-template>
        <ng-template hhDataTableCell="actions" let-row>
          <a mat-icon-button [routerLink]="['/pharmacy/prescriptions', idOf(row)]"
...
export class PrescriptionListComponent implements OnInit, OnDestroy {
  ...
  columns: HisHopeDataTableColumn[] = [
    { key: 'id', label: this.i18n.t('prescriptions.column.id', 'Mã đơn'), sortable: true, width: '120px' },
    { key: 'medicationName', label: this.i18n.t('prescriptions.column.medicationName', 'Thuốc') },
    { key: 'strength', label: this.i18n.t('prescriptions.column.strength', 'Hàm lượng') },
    { key: 'quantity', label: this.i18n.t('prescriptions.column.quantity', 'Số lượng'), align: 'end' as const },
    { key: 'prescribedAt', label: this.i18n.t('prescriptions.column.prescribedAt', 'Ngày kê'), sortable: true },
    { key: 'statusCode', label: this.i18n.t('prescriptions.column.status', 'Trạng thái'), status: true },
  ...
  loadData(): void {
    ...
    this.pharmacyService.searchPrescriptions({
      ...
      error: () => {
        this.error = this.i18n.t('prescriptions.loadFailed', 'Không thể tải danh sách đơn thuốc.');
        this.loading = false;
        this.cdr.markForCheck();
      },
    });
