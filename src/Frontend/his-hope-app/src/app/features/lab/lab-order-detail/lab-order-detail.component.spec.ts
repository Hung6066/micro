...
describe("LabOrderDetailComponent", () => {
  ...
  beforeEach(async () => {
    const spy = jasmine.createSpyObj("LabService", [
      "getLabOrder",
      "submitLabOrder",
      "collectSpecimen",
      "cancelLabOrder",
    ...
    alertSpy = jasmine.createSpyObj("LabCriticalAlertService", [
      "listCriticalAlerts",
      "acknowledgeCriticalAlert",
    ]);
    alertSpy.listCriticalAlerts.and.returnValue(of([mockAlert] as any));
    alertSpy.acknowledgeCriticalAlert.and.returnValue(
      of({ ...mockAlert, status: "ACKNOWLEDGED" } as any),
    );
    ...
    ) as jasmine.SpyObj<LabCriticalAlertService>;
    TestBed.inject(HisHopePermissionService).setPermissions([
      "lab.alert.acknowledge",
    ]);
  ...
  it("should acknowledge a critical alert from the detail panel", () => {
    ...
    expect(alertService.acknowledgeCriticalAlert).toHaveBeenCalledWith(
      "alert-1",
    );
  });
