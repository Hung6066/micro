...
import {
  CriticalAlert,
  CriticalAlertStatus,
} from "@core/models/critical-alert.model";
    ...
    <div class="lab-critical-alerts">
      <div class="header">
        ...
        <div class="header-actions">
          <a mat-stroked-button routerLink="/lab/critical-alerts/rules"
            >Quy tắc</a
          >
          <a mat-raised-button color="primary" routerLink="/lab"
            >Phiếu xét nghiệm</a
          >
        </div>
      </div>
      ...
      <div
        class="filters"
        role="tablist"
        aria-label="Bộ lọc cảnh báo nghiêm trọng"
      >
      ...
      @if (visibleAlerts.length === 0) {
        <mat-card>
          <mat-card-content
            >Không có cảnh báo nào trong bộ lọc này.</mat-card-content
          >
        </mat-card>
      }
        ...
        @for (alert of visibleAlerts; track alert.id) {
          <mat-card class="alert-card">
            <mat-card-header>
              ...
              <mat-card-subtitle
                >{{ alert.resultValue }}
                {{ alert.resultUnit }}</mat-card-subtitle
              >
            </mat-card-header>
            <mat-card-content>
              <p>
              ...
              <p>
                <strong>Trạng thái:</strong> {{ statusLabel(alert.status) }}
              </p>
            </mat-card-content>
            <mat-card-actions>
              <button
              ...
              <a mat-button [routerLink]="['/lab/critical-alerts', alert.id]"
                >Chi tiết</a
              >
            </mat-card-actions>
          </mat-card>
...
export class LabCriticalAlertsComponent implements OnInit, OnDestroy {
  ...
  readonly filters: Array<{
    status: CriticalAlertStatus | "ALL";
    label: string;
  }> = [
