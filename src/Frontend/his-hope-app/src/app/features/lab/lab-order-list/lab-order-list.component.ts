  ...
  template: `
    <hh-page-layout>
      <hh-page-header
        ...
        [subtitle]="
          'labOrders.subtitle'
            | hhTranslate: 'Quản lý phiếu xét nghiệm và mẫu bệnh phẩm'
        "
      >
        <span class="critical-badge" aria-live="polite"
          >{{ unreadCriticalAlertCount }}
          {{ "labOrders.criticalAlert" | hhTranslate: "cảnh báo mới" }}</span
        >
        <a mat-stroked-button routerLink="/lab/critical-alerts">{{
          "labOrders.alertBox" | hhTranslate: "Hộp cảnh báo"
        }}</a>
        <button
          mat-raised-button
          color="primary"
          routerLink="/lab/new"
          [attr.aria-label]="
            'labOrders.new' | hhTranslate: 'Tạo phiếu xét nghiệm mới'
          "
        >
          <mat-icon>add</mat-icon>
          {{ "labOrders.new" | hhTranslate: "Tạo phiếu xét nghiệm" }}
        </button>
      </hh-page-header>

      <hh-toolbar
        hhPageToolbar
        [label]="'labOrders.toolbar' | hhTranslate: 'Phiếu xét nghiệm'"
      >
        <span hhToolbarTitle
          >{{ totalItems }}
          {{ "labOrders.count" | hhTranslate: "phiếu xét nghiệm" }}</span
        >
        <label class="hh-status-filter" hh-toolbar-controls>
          ...
          <select
            ...
            [attr.aria-label]="
              'labOrders.statusFilter' | hhTranslate: 'Lọc theo trạng thái'
            "
          >
            <option value="">{{ "common.all" | hhTranslate: "Tất cả" }}</option>
            <option value="ordered">
              {{ "labOrders.status.ordered" | hhTranslate: "Đã chỉ định" }}
            </option>
            <option value="specimen_collected">
              {{ "labOrders.status.collected" | hhTranslate: "Đã lấy mẫu" }}
            </option>
            <option value="in_progress">
              {{ "labOrders.status.inProgress" | hhTranslate: "Đang xử lý" }}
            </option>
            <option value="completed">
              {{ "labOrders.status.completed" | hhTranslate: "Hoàn thành" }}
            </option>
            <option value="cancelled">
              {{ "labOrders.status.cancelled" | hhTranslate: "Đã hủy" }}
            </option>
          </select>
        </label>
      </hh-toolbar>
      ...
      <hh-data-table
        [label]="'labOrders.tableLabel' | hhTranslate: 'Phiếu xét nghiệm'"
        [loading]="loading"
        [error]="error"
        [empty]="rows.length === 0 && !loading"
        [emptyMessage]="
          'labOrders.empty'
            | hhTranslate: 'Không tìm thấy phiếu xét nghiệm nào.'
        "
        [searchPlaceholder]="
          'labOrders.search' | hhTranslate: 'Mã phiếu, bệnh nhân...'
        "
        [columns]="columns"
        [rows]="rows"
        [pageSize]="20"
      ...
      >
        ...
        <ng-template hhDataTableCell="orderDate" let-row>{{
          row["orderDate"] | date: "mediumDate"
        }}</ng-template>
        <ng-template hhDataTableCell="priorityName" let-row>
          <span
            class="priority-badge"
            [class.priority-high]="priorityOf(row) === 'high'"
        ...
        <ng-template hhDataTableCell="testsCount" let-row>{{
          testsCountOf(row)
        }}</ng-template>
...
export class LabOrderListComponent implements OnInit, OnDestroy {
  ...
  columns: HisHopeDataTableColumn[] = [
    {
      key: "id",
      label: this.i18n.t("labOrders.column.id", "Mã phiếu"),
      sortable: true,
      width: "120px",
    },
    {
      key: "patientName",
      label: this.i18n.t("labOrders.column.patient", "Bệnh nhân"),
    },
    {
      key: "orderDate",
      label: this.i18n.t("labOrders.column.orderDate", "Ngày chỉ định"),
      sortable: true,
    },
    {
      key: "priorityName",
      label: this.i18n.t("labOrders.column.priority", "Mức ưu tiên"),
    },
    {
      key: "testsCount",
      label: this.i18n.t("labOrders.column.testsCount", "Số xét nghiệm"),
      align: "end" as const,
    },
    {
      key: "statusCode",
      label: this.i18n.t("labOrders.column.status", "Trạng thái"),
      status: true,
    },
    {
      key: "actions",
  ...
  ngOnInit(): void {
    ...
    this.criticalAlertStreamService.latestAlert$
      ...
      .subscribe((alert) => {
        ...
        this.snackBar.open(
          this.i18n.t(
            "labOrders.criticalAlertToast",
            "Có cảnh báo xét nghiệm nghiêm trọng mới",
          ),
      ...
      .subscribe({
        ...
        error: () => {
          this.error = this.i18n.t(
            ...
            "Không thể tải danh sách phiếu xét nghiệm.",
          );
          this.loading = false;
          this.cdr.markForCheck();
        },
