...
import {
  ...
  StartEncounterData,
} from "./dialogs/start-encounter.dialog";
import {
  OrderLabDialogComponent,
  OrderLabData,
} from "./dialogs/order-lab.dialog";
import {
  PrescribeDialogComponent,
  PrescribeData,
} from "./dialogs/prescribe.dialog";
import {
  ScheduleDialogComponent,
  ScheduleData,
} from "./dialogs/schedule.dialog";
import {
  RecordPaymentDialogComponent,
  RecordPaymentData,
} from "./dialogs/record-payment.dialog";
import {
...
export class PatientWorkspaceComponent implements OnInit, OnDestroy {
  ...
  allergyTooltip(a: any): string {
    ...
    return (
      this.i18n.t("patient.allergyReaction", "Phản ứng: {{reaction}}", {
        reaction,
      }) +
      " | " +
      this.i18n.t("patient.allergySeverity", "Mức độ: {{severity}}", {
        severity,
      })
    );
  }
  ...
  conditionTooltip(c: any): string {
    ...
    return (
      chronicity +
      " | " +
      this.i18n.t("patient.icd10", "ICD-10: {{code}}", { code })
    );
  }
  ...
  private loadEncounters(): void {
    ...
    this.patientService
      .getEncounters(this.patientId, 1, 20)
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: (res) => {
  ...
  private loadAppointments(): void {
    ...
    this.patientService
      .getAppointments(this.patientId, 1, 20)
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: (res) => {
  ...
  private loadLabOrders(): void {
    ...
    this.patientService
      .getLabOrders(this.patientId, 1, 20)
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: (res) => {
  ...
  private loadPrescriptions(): void {
    ...
    this.patientService
      .getPrescriptions(this.patientId, 1, 20)
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: (res) => {
  ...
  private loadInvoices(): void {
    ...
    this.patientService
      .getInvoices(this.patientId, 1, 20)
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: (res) => {
  ...
  checkInAppointment(id: string): void {
    this.appointmentService
      ...
      .subscribe({
        next: () => {
          this.snackBar.open(
            this.i18n.t("patients.checkInSuccess", "Đã check-in lịch hẹn"),
        ...
        error: () =>
          this.snackBar.open(
            this.i18n.t("patients.checkInFailed", "Không thể check-in"),
            this.i18n.t("common.close", "Đóng"),
            { duration: 5000 },
          ),
      });
  ...
  checkOutAppointment(id: string): void {
    this.appointmentService
      ...
      .subscribe({
        next: () => {
          this.snackBar.open(
            this.i18n.t("patients.checkOutSuccess", "Đã check-out lịch hẹn"),
            this.i18n.t("common.close", "Đóng"),
            { duration: 3000 },
          );
          this.loadAppointments();
        ...
        error: () =>
          this.snackBar.open(
            this.i18n.t("patients.checkOutFailed", "Không thể check-out"),
            this.i18n.t("common.close", "Đóng"),
            { duration: 5000 },
          ),
      });
  ...
  cancelAppointment(id: string): void {
    this.appointmentService
      ...
      .subscribe({
        next: () => {
          this.snackBar.open(
            this.i18n.t("patients.cancelAppointmentSuccess", "Đã hủy lịch hẹn"),
            this.i18n.t("common.close", "Đóng"),
            { duration: 3000 },
          );
          this.loadAppointments();
        ...
        error: () =>
          this.snackBar.open(
            this.i18n.t(
              "patients.cancelAppointmentFailed",
  ...
  hasAbnormalFlag(order: LabOrder): boolean {
    return (
      order.tests?.some(
        (t) =>
          t.result?.abnormalFlagCode &&
          t.result.abnormalFlagCode !== "none" &&
          t.result.abnormalFlagCode !== "normal",
      ) ?? false
    );
  }
