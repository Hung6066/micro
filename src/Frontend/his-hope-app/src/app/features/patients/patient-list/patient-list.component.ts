  ...
  template: `
    <hh-page-layout>
      ...
      <hh-data-table
        ...
        [rowClickable]="true"
        (rowClick)="viewPatient(patientFromRow($event).id)"
        (retry)="loadPatients()"
      >
        <ng-template hhDataTableCell="dateOfBirth" let-row>{{
          row["dateOfBirth"] | date: "mediumDate"
        }}</ng-template>
        <ng-template hhDataTableCell="actions" let-row>
          <button
            mat-icon-button
            color="primary"
            (click)="viewPatient(patientFromRow(row).id)"
          ...
          <button
            mat-icon-button
            color="accent"
            [routerLink]="['/patients', patientFromRow(row).id, 'edit']"
...
export class PatientListComponent implements OnInit, OnDestroy {
  ...
  patientFromRow(row: Record<string, unknown>): Patient {
    return row["entity"] as Patient;
  }
