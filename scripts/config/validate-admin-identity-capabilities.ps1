...
$en = Join-Path $root 'shared\frontend-foundation\i18n\src\dictionaries\en.ts'
$vi = Join-Path $root 'shared\frontend-foundation\i18n\src\dictionaries\vi-vn.ts'
...
$adminApi = Join-Path $AdminAppRoot 'src\app\core\services\identity-operations-api.service.ts'
...
Require-Text $provisioningEndpoint '(Permission:admin\.users\.write|AuthorizationPolicyNames\.Permissions\.(AdminUsersWrite|AdminProvisioningManage))' 'provisioning mutation routes must use the write permission'
