...
$sharedApplicationOverlay = Read-RepoFile 'k8s/overlays/prod-shared-storage/kustomization.yaml'
...
if ($sharedApplicationOverlay -and $sharedApplicationOverlay -match "value:\s*$([regex]::Escape($StorageClassName))" -and
    $sharedDataOverlay -and $sharedDataOverlay -match "value:\s*$([regex]::Escape($StorageClassName))") {
...
$prodKustomization = $sharedApplicationOverlay
...
if ($sharedApplicationOverlay -and $sharedDataOverlay -and $sharedObservabilityOverlay -and
    $sharedApplicationOverlay -match "value:\s*$([regex]::Escape($StorageClassName))" -and
...
if ($sharedApplicationOverlay -and $sharedApplicationOverlay -match "value:\s*$([regex]::Escape($StorageClassName))") {
...
} elseif ($snapshotFiles.Count -gt 0) {
