...
-- NOTE: This migration depends on tables created in migrations 001-008
--       and the identity extensions (Permissions, AspNetRoles, etc.)
--       created in migration 013. Role IDs used here match 013's seed data.
-- ============================================================================

-- ============================================================================
-- PREAMBLE: Column additions and role seed data provided by migration 013
-- ============================================================================
-- Migration 013-identity-extensions.sql handles:
--   - Adding FirstName, LastName, MiddleName, SecurityStamp, ConcurrencyStamp,
--     PhoneNumberConfirmed, LockoutEnd, FacilityId to AspNetUsers
--   - Creating Permissions, AspNetRoles, AspNetUserRoles, RolePermissions tables
--   - Seeding roles with stable IDs (00000000-... convention)
--
-- This migration depends on 013 having run first. The role inserts below
-- are kept as idempotent fallbacks (ON CONFLICT DO NOTHING) for safety.

-- ============================================================================
-- PREAMBLE: Ensure roles exist (idempotent, matches 013 seed)
-- ============================================================================
-- If migration 013 hasn't run yet, these inserts create the minimum roles
-- needed for the user seed data. If 013 has already run, ON CONFLICT skips.

INSERT INTO identitydb.AspNetRoles (Id, Name, NormalizedName, Description, IsSystem, CreatedAt, ConcurrencyStamp) VALUES
('00000000-0000-0000-0000-000000000001', 'Admin', 'ADMIN', 'Quản trị viên hệ thống', true, now(), gen_random_uuid()::STRING),
('00000000-0000-0000-0000-000000000002', 'Provider', 'PROVIDER', 'Bác sĩ', true, now(), gen_random_uuid()::STRING),
('00000000-0000-0000-0000-000000000003', 'Nurse', 'NURSE', 'Điều dưỡng', true, now(), gen_random_uuid()::STRING),
('00000000-0000-0000-0000-000000000004', 'Receptionist', 'RECEPTIONIST', 'Lễ tân', true, now(), gen_random_uuid()::STRING),
('00000000-0000-0000-0000-000000000005', 'LabTechnician', 'LABTECHNICIAN', 'Kỹ thuật viên xét nghiệm', true, now(), gen_random_uuid()::STRING),
('00000000-0000-0000-0000-000000000006', 'Pharmacist', 'PHARMACIST', 'Dược sĩ', true, now(), gen_random_uuid()::STRING),
('00000000-0000-0000-0000-000000000007', 'BillingClerk', 'BILLINGCLERK', 'Nhân viên thanh toán', true, now(), gen_random_uuid()::STRING)
...
INSERT INTO identitydb.AspNetUsers (Id, UserName, NormalizedUserName, Email, NormalizedEmail, EmailConfirmed, PasswordHash, PhoneNumber, TwoFactorEnabled, LockoutEnabled, AccessFailedCount, FullName, FirstName, LastName, MiddleName, Role, FacilityId, IsActive, CreatedAt, SecurityStamp, ConcurrencyStamp) VALUES
-- Admin — Quản Trị Viên
('00000000-0000-0000-0000-000000000101', 'admin@hishop.vn', 'ADMIN@HISHOP.VN', 'admin@hishop.vn', 'ADMIN@HISHOP.VN', true, 'AQAAAAIAAYagAAAAEIvMJiFkRNflIyTrS1SxY7x6mHqQo7/JQ2MJxOZ8zKxCv/9QbM3UJmKPfMq0L0KJqg==', '0901000101', false, true, 0, 'Quản Trị Viên', 'Quản Trị Viên', NULL, NULL, 'Admin', '11111111-1111-1111-1111-111111111111', true, '2026-06-15 08:00:00+00', gen_random_uuid()::STRING, gen_random_uuid()::STRING),
-- Bác sĩ Nguyễn Văn Minh
('00000000-0000-0000-0000-000000000102', 'bacsy.nguyen@hishop.vn', 'BACSY.NGUYEN@HISHOP.VN', 'bacsy.nguyen@hishop.vn', 'BACSY.NGUYEN@HISHOP.VN', true, 'AQAAAAIAAYagAAAAEIvMJiFkRNflIyTrS1SxY7x6mHqQo7/JQ2MJxOZ8zKxCv/9QbM3UJmKPfMq0L0KJqg==', '0901000102', false, true, 0, 'Nguyễn Văn Minh', 'Minh', 'Nguyễn', 'Văn', 'Provider', '11111111-1111-1111-1111-111111111111', true, '2026-06-15 08:00:00+00', gen_random_uuid()::STRING, gen_random_uuid()::STRING),
-- Bác sĩ Trần Thị Lan
('00000000-0000-0000-0000-000000000103', 'bacsy.tran@hishop.vn', 'BACSY.TRAN@HISHOP.VN', 'bacsy.tran@hishop.vn', 'BACSY.TRAN@HISHOP.VN', true, 'AQAAAAIAAYagAAAAEIvMJiFkRNflIyTrS1SxY7x6mHqQo7/JQ2MJxOZ8zKxCv/9QbM3UJmKPfMq0L0KJqg==', '0901000103', false, true, 0, 'Trần Thị Lan', 'Lan', 'Trần', 'Thị', 'Provider', '11111111-1111-1111-1111-111111111111', true, '2026-06-15 08:00:00+00', gen_random_uuid()::STRING, gen_random_uuid()::STRING),
-- Điều dưỡng Lê Thị Hồng
('00000000-0000-0000-0000-000000000104', 'dieuduong.le@hishop.vn', 'DIEUDUONG.LE@HISHOP.VN', 'dieuduong.le@hishop.vn', 'DIEUDUONG.LE@HISHOP.VN', true, 'AQAAAAIAAYagAAAAEIvMJiFkRNflIyTrS1SxY7x6mHqQo7/JQ2MJxOZ8zKxCv/9QbM3UJmKPfMq0L0KJqg==', '0901000104', false, true, 0, 'Lê Thị Hồng', 'Hồng', 'Lê', 'Thị', 'Nurse', '11111111-1111-1111-1111-111111111111', true, '2026-06-15 08:00:00+00', gen_random_uuid()::STRING, gen_random_uuid()::STRING)
ON CONFLICT (Id) DO NOTHING;

-- ============================================================================
-- SECTION 1b: Seed User-Role assignments (AspNetUserRoles)
-- ============================================================================
-- Maps each user to their role using the same role IDs from migration 013.

INSERT INTO identitydb.AspNetUserRoles (UserId, RoleId, AssignedAt) VALUES
-- Admin → Admin role
('00000000-0000-0000-0000-000000000101', '00000000-0000-0000-0000-000000000001', now()),
-- Nguyễn Văn Minh → Provider role
('00000000-0000-0000-0000-000000000102', '00000000-0000-0000-0000-000000000002', now()),
-- Trần Thị Lan → Provider role
('00000000-0000-0000-0000-000000000103', '00000000-0000-0000-0000-000000000002', now()),
-- Lê Thị Hồng → Nurse role
('00000000-0000-0000-0000-000000000104', '00000000-0000-0000-0000-000000000003', now())
...
-- 2a. Ensure FacilityId and CreatedBy columns exist on Patients (for RLS)
...
-- 2b. Patients (8 patients with Vietnamese names)
-- NOTE: INSERT now includes FacilityId and CreatedBy columns.
--      FacilityId matches the hospital facility from identity seed.
...
-- 2c. Allergies
INSERT INTO patientdb.Allergies (PatientId, AllergyId, Allergen, Reaction, Severity, RecordedDate, IsActive) VALUES
...
-- 2d. Medical Conditions
INSERT INTO patientdb.MedicalConditions (PatientId, ConditionId, ConditionName, Icd10Code, OnsetDate, ResolvedDate, IsChronic, Notes, RecordedDate, IsActive) VALUES
-- P001: Nguyễn Văn A
...
-- 2e. Patient OutboxMessages (auto-confirmed events for seed data)
INSERT INTO patientdb.OutboxMessages (Id, Type, Content, CorrelationId, CausationId, OccurredOn, ProcessedOn, Status, Error, RetryCount) VALUES
...
INSERT INTO appointmentdb.Appointments (AppointmentId, PatientId, ProviderId, FacilityId, ScheduledDate, DurationMinutes, Status, Reason, Notes, CheckInAt, CheckOutAt, CanceledAt, CancelReason, CreatedAt, UpdatedAt) VALUES
...
INSERT INTO appointmentdb.OutboxMessages (Id, Type, Content, CorrelationId, CausationId, OccurredOn, ProcessedOn, Status) VALUES
...
-- 4a. Ensure FacilityId column exists on Encounters (for RLS)
ALTER TABLE clinicaldb.Encounters ADD COLUMN IF NOT EXISTS FacilityId UUID;

-- 4b. Encounters (6 clinical encounters with SOAP notes in Vietnamese)
INSERT INTO clinicaldb.Encounters (EncounterId, PatientId, AppointmentId, ProviderId, EncounterDate, EncounterType, Status, ChiefComplaint,
    ...
    Assessment, Plan, DiagnosisNotes, CreatedAt, UpdatedAt, FacilityId) VALUES
...
Đái tháo đường type 2 (E11) - ổn định',
 '2026-07-15 08:05:00+00', '2026-07-15 08:30:00+00', '11111111-1111-1111-1111-111111111111'),
...
6. Tái khám sau 3 ngày nếu không đỡ',
 ...
 '2026-07-16 09:30:00+00', NULL, '11111111-1111-1111-1111-111111111111'),
...
5. Tái khám sau 1 tháng có kết quả HbA1c',
 ...
 '2026-07-17 14:00:00+00', NULL, '11111111-1111-1111-1111-111111111111'),
...
5. Nếu nặng hơn: nhập viện',
 ...
 '2026-07-16 08:00:00+00', NULL, '11111111-1111-1111-1111-111111111111'),
...
Trào ngược dạ dày thực quản (K21.9)',
 '2026-07-15 10:00:00+00', NULL, '11111111-1111-1111-1111-111111111111'),
...
Thừa cân (R63.5) - BMI 27',
 '2026-07-15 15:00:00+00', '2026-07-15 15:40:00+00', '11111111-1111-1111-1111-111111111111')
ON CONFLICT (EncounterId) DO NOTHING;

-- 4c. EncounterDiagnoses (child table of Encounters)
INSERT INTO clinicaldb.EncounterDiagnoses (EncounterId, Id, ConditionName, Icd10Code, IsPrimary, Notes) VALUES
-- E001: Nguyễn Văn A
...
-- 4d. EncounterProcedures (child table of Encounters)
INSERT INTO clinicaldb.EncounterProcedures (EncounterId, Id, ProcedureName, CptCode, PerformedDate, Notes) VALUES
...
-- 4e. Clinical Notes (Progress notes in Vietnamese)
INSERT INTO clinicaldb.ClinicalNotes (EncounterId, NoteId, NoteType, Content, RecordedAt, RecordedBy) VALUES
...
-- 4f. Clinical Prescriptions (linked to encounters)
INSERT INTO clinicaldb.Prescriptions (EncounterId, PrescriptionId, MedicationName, Dosage, Frequency, Route, DurationDays, PrescribedAt, PrescribedBy, Instructions, IsActive) VALUES
...
INSERT INTO clinicaldb.OutboxMessages (Id, Type, Content, CorrelationId, OccurredOn, ProcessedOn, Status) VALUES
