...
export class SalesAllocationPageComponent implements OnInit {
  ...
  allocate(): void { const uuid = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i; if (!this.draft.sku.trim() || !uuid.test(this.draft.salesOrderId.trim()) || this.draft.quantity <= 0) { this.actionError = this.i18n.t("customerPortal.allocationFormInvalid", "SKU, valid sales order ID and positive quantity are required."); return; } this.busy = true; this.actionError = ""; this.api.allocateSales(this.draft.sku.trim(), { salesOrderId: this.draft.salesOrderId.trim(), quantity: this.draft.quantity }).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({ next: (result) => { this.allocation = result; this.busy = false; this.loadAvailability(); this.cdr.markForCheck(); }, error: (error) => { this.actionError = this.errors.message(error, "customerPortal.allocationFailed"); this.busy = false; this.cdr.markForCheck(); } }); }
}
