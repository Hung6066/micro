...
@Component({
  ...
  imports: [DecimalPipe, FormsModule, HisHopeActionButtonComponent, HisHopePageHeaderComponent, HisHopePageLayoutComponent, HisHopeStateComponent, HisHopeTranslatePipe],
  template: `
    <hh-page-layout>
          ...
          <form class="allocation-form" (ngSubmit)="allocate()">
            ...
            <label>{{ 'customerPortal.salesOrder' | hhTranslate: 'Sales order' }}
              <select name="orderId" [(ngModel)]="draft.salesOrderId" required>
                ...
                @for (order of orders; track order.id) { <option [value]="order.id">{{ order.lines[0]?.name || order.lines[0]?.sku || ('customerPortal.unnamedOrder' | hhTranslate: 'Sales order') }} · {{ buyerLabel(order.buyerUserId) }} · {{ order.status }} · {{ order.totalAmount | number:'1.0-0' }}</option> }
              </select>
            </label>
...
export class SalesAllocationPageComponent implements OnInit {
  ...
  availability: HisHopeManufacturingAvailabilityDto | null = null; allocation: HisHopeManufacturingSalesAllocationDto | null = null; orders: HisHopeCommerceOrderDto[] = []; users: OperatorDirectoryUser[] = []; loading = true; busy = false; error = ""; actionError = ""; tenantLabel: string | null = null;
  ...
  allocate(): void { const uuid = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i; if (!this.draft.sku.trim() || !uuid.test(this.draft.salesOrderId.trim()) || this.draft.quantity <= 0) { this.actionError = this.i18n.t("customerPortal.allocationFormInvalid", "SKU, valid sales order ID and positive quantity are required."); return; } this.busy = true; this.actionError = ""; this.api.allocateSales(this.draft.sku.trim(), { salesOrderId: this.draft.salesOrderId.trim(), quantity: this.draft.quantity }).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({ next: (result) => { this.allocation = result; this.busy = false; this.loadAvailability(); this.cdr.markForCheck(); }, error: (error) => { this.actionError = this.errors.message(error, "customerPortal.allocationFailed"); this.busy = false; this.cdr.markForCheck(); } }); }
}
