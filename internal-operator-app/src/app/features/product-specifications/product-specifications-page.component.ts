...
@Component({
  ...
  template: `
    <hh-page-layout>
      ...
      <section class="workflow-reference" data-testid="product-specification-workflow-reference">
        <h2 class="workflow-reference__title">{{ "customerPortal.workflowProductSpecification" | hhTranslate: "Product specification lifecycle" }}</h2>
        <hh-workflow-stepper [ariaLabel]="'customerPortal.workflowProductSpecification' | hhTranslate: 'Product specification lifecycle'" [steps]="referenceWorkflowSteps" />
      </section>
        ...
        <section class="section spec-grid">
          @for (spec of specifications; track spec.id) {
            <article class="card">
              ...
              <hh-workflow-stepper class="entity-workflow" [attr.data-testid]="'product-specification-workflow-' + spec.id" [ariaLabel]="'customerPortal.workflowProductSpecification' | hhTranslate: 'Product specification lifecycle'" [steps]="specificationWorkflowSteps(spec.status)" />
...
export class ProductSpecificationsPageComponent implements OnInit, AfterViewInit {
  ...
  specificationStatusLabel(status: string): string { return portalEnumLabel(this.i18n, "governanceLifecycleStatus", status); }
  get referenceWorkflowSteps() { return buildReferenceWorkflowSteps(this.i18n, "product-specification"); }
  specificationWorkflowSteps(status: string) { return buildEntityWorkflowSteps(this.i18n, "product-specification", status); }
