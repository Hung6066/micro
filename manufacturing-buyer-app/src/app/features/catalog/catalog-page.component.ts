...
import { CommerceApiService, ProductCatalogItem } from "../../core/services/commerce-api.service";
...
export class CatalogPageComponent implements OnInit {
  ...
  filtered: ProductCatalogItem[] = [];
  loading = true;
  error = "";
  availability: Record<string, { quantity: number; uom: string }> = {};
  searchQuery = "";
  ...
  ngOnInit(): void {
    ...
    this.api.getProducts().pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
        ...
        if (this.products.length) {
          forkJoin(
            this.products.map((product) =>
              this.api.getAvailability(product.sku).pipe(
                catchError(() => of(null)),
              ),
            ),
          )
            ...
            .subscribe((availability) => {
              this.availability = Object.fromEntries(
                availability
                  .filter((item): item is NonNullable<typeof item> => item !== null)
                  .map((item) => [item.sku, { quantity: item.releasedQuantity, uom: item.uom }]),
              );
              this.cdr.markForCheck();
            });
        }
      ...
      error: (error) => {
        this.error = this.errors.message(error, "buyer.catalog.error");
        this.loading = false;
        this.cdr.markForCheck();
      },
    });
  ...
  categoryLabel(product: ProductCatalogItem): string {
    return productCategoryLabel(product.sku);
  }

  imageUrl(product: ProductCatalogItem): string {
    return productImageUrl(product.sku);
  }

  displayPrice(product: ProductCatalogItem): number {
    return product.effectiveUnitPrice;
  }

  availableQuantity(product: ProductCatalogItem): string {
    const item = this.availability[product.sku];
    return item ? `${item.quantity} ${item.uom}` : "—";
  }
