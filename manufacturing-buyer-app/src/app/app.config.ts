...
export function createBuyerRuntimeConfig(source = defaultRuntimeSource()) {
...
const runtime = createBuyerRuntimeConfig();
  ...
  providers: [
    {
      ...
      useFactory: (document: Document, platformId: object) => {
        const storedLocale = document.defaultView?.localStorage?.getItem("hh-locale");
        const i18n = new HisHopeI18nService(document, platformId);
        for (const [locale, translations] of Object.entries(buyerTranslations)) {
          i18n.registerTranslations(locale, translations);
        }
        i18n.registerLocale("en-US", i18n.dictionaries["en"]);
        if (storedLocale) i18n.setLocale(storedLocale === "en" ? "en-US" : storedLocale, false);
        return i18n;
      },
